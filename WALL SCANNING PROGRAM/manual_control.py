...
# Limit switch pins
LIMIT_LEFT = 14    # X positive limit
LIMIT_RIGHT = 15   # X negative limit
LIMIT_UP = 17      # Y positive limit
LIMIT_DOWN = 19    # Y negative limit

# Motor speed (PWM frequency in Hz)
MOTOR_SPEED = 3000  # Adjust this for faster/slower movement
# --------------------------
...
class WallScannerController:
    ...
    def stop_motors(self):
        """Stop PWM on both motor step pins and reset DIR pins to LOW"""
        pi.set_PWM_dutycycle(STEP1, 0)
        pi.set_PWM_dutycycle(STEP2, 0)
        # Reset direction pins to LOW (default state)
        pi.write(DIR1, 0)
        pi.write(DIR2, 0)
...
class ControlGUI:
    ...
    def update_limit_status(self):
        ...
        limits = []
        try:
            if GPIO.input(LIMIT_UP) == GPIO.LOW:
                limits.append("UP")
            if GPIO.input(LIMIT_DOWN) == GPIO.LOW:
                limits.append("DOWN")
            if GPIO.input(LIMIT_LEFT) == GPIO.LOW:
                limits.append("LEFT")
            if GPIO.input(LIMIT_RIGHT) == GPIO.LOW:
                limits.append("RIGHT")
