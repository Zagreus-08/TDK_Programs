...
def is_outlook_running():
    """
    Detects both Classic Outlook and New Outlook.
    """
    for proc in psutil.process_iter(['name', 'cmdline']):
        try:
            ...
            # New Outlook (Store / WebView-based)
            if name in ("olk.exe", "msedgewebview2.exe"):
                if "outlook" in cmdline:
                    return True
...
def _create_esd_plot_images(dates, wrist_by_emp, shoe_by_emp):
    ...
    try:
        ...
        def make_row_image(data_by_emp, title, ucl, base_ylim):
            # Create figure sized for email display (1200px wide at 150 DPI = 8 inches)
            ...
            for name, series in sorted(data_by_emp.items()):
                ...
                for week_dates in week_groups:
                    ...
                    if len(week_x) > 0:
                        line, = ax.plot(week_x, week_y, marker='o', markersize=3, 
            ...
            ax.set_xticklabels(x_labels, rotation=45, ha='right', fontsize=7, weight='normal')
            
            # Increase tick label sizes for y-axis too
            ...
            ax.axhline(ucl, color='red', linewidth=2.5, zorder=10, alpha=0.95)
            ...
            # Save with high DPI for sharp quality at reasonable size
            buf = io.BytesIO()
            # 16" x 150 DPI = 2400px wide (good for email display)
            fig.savefig(buf, format='png', bbox_inches='tight', dpi=150, 
                       facecolor='white', edgecolor='none',
                       pil_kwargs={'quality': 95, 'optimize': False})
            plt.close(fig)
            buf.seek(0)
            return buf.read()
...
def send_absentee_email(absentees):
    ...
    body_text = f"""
    <p>The following personnel <b>did not enter the Production Area / Perform ESD Wrist Strap and Shoe Testing</b> on {display_date}:</p>
    <ul>
