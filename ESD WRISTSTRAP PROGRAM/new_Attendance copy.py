...
def _create_esd_plot_image(dates, wrist_by_emp, shoe_by_emp):
    ...
    try:
        ...
        buf = io.BytesIO()
        fig.savefig(buf, format='png', bbox_inches='tight', dpi=1000)
        plt.close(fig)
        buf.seek(0)
        return buf.read()
    except Exception:
...
def _create_esd_plot_images(dates, wrist_by_emp, shoe_by_emp):
    ...
    try:
        ...
        def make_row_image(data_by_emp, title, ucl, base_ylim):
            # Create extremely large figure for maximum resolution
            ...
            for name, series in sorted(data_by_emp.items()):
                ...
                for week_dates in week_groups:
                    ...
                    if len(week_x) > 0:
                        line, = ax.plot(week_x, week_y, marker='o', markersize=4.5, 
                                       linewidth=1.5, alpha=0.9, zorder=5, color=emp_color,
                                       antialiased=True)
            ...
            ax.set_xticklabels(x_labels, rotation=45, ha='right', fontsize=14, weight='normal')
            ...
            ax.axhline(ucl, color='red', linewidth=3.5, zorder=10, alpha=0.95)
            
            # Title at top
            ax.set_title(title, fontweight='bold', fontsize=24, pad=30)
