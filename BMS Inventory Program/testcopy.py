...
class InventoryApp:
    ...
    def open_pr_monitor_window(self):
        ...
        def edit_pr():
            ...
            c = conn.cursor()
            c.execute("""
                SELECT pr.id, pr.material_id, m.description, pr.purpose, pr.quantity, pr.quantity_remaining,
                       pr.requestor, pr.status, pr.po_number, pr.estimated_delivery_date, pr.date_requested
                FROM purchase_requests pr
                JOIN materials m ON pr.material_id = m.id
                WHERE pr.id = ?
            """, (pid,))
            ...
            (pr_id, material_id, material_desc, purpose, quantity, quantity_remaining,
             requestor, status, po_number, etd, date_requested) = row
            ...
            frm = ttk.Frame(dlg, style="Dark.TFrame")
            frm.pack(fill="both", expand=True, padx=12, pady=12)

            # Material combo
            tk.Label(frm, text="Material:", bg=BACKGROUND_GRAY, fg=TEXT_DARK, font=FONT).grid(row=0, column=0, sticky="w")
            mat_var = tk.StringVar()
            mat_cb = ttk.Combobox(frm, textvariable=mat_var, style="Dark.TCombobox", width=60)
            mat_cb.grid(row=0, column=1, sticky="ew", pady=4)

            # load materials list
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("SELECT id, description FROM materials ORDER BY id")
            mats = c.fetchall()
            conn.close()
            choices = [f"{m[0]} - {m[1]}" for m in mats]
            id_from_display_local = {f"{m[0]} - {m[1]}": m[0] for m in mats}
            mat_cb['values'] = choices
            # set current material selection
            current_display = next((k for k,v in id_from_display_local.items() if v == material_id), None)
            if current_display:
                mat_cb.set(current_display)
            elif choices:
                mat_cb.set(choices[0])

            def filter_mat(event):
                typed = mat_var.get().lower()
                filtered = [item for item in choices if typed in item.lower()]
                mat_cb['values'] = filtered
            mat_cb.bind("<KeyRelease>", filter_mat)

            # Purpose
            tk.Label(frm, text="Purpose:", bg=BACKGROUND_GRAY, fg=TEXT_DARK, font=FONT).grid(row=1, column=0, sticky="w")
            purpose_var = tk.StringVar(value=purpose or "")
            ttk.Entry(frm, textvariable=purpose_var, style="Dark.TEntry", width=60).grid(row=1, column=1, sticky="ew", pady=4)

            # Quantity
            tk.Label(frm, text="Quantity (Total):", bg=BACKGROUND_GRAY, fg=TEXT_DARK, font=FONT).grid(row=2, column=0, sticky="w")
            qty_var = tk.IntVar(value=quantity or 0)
            ttk.Entry(frm, textvariable=qty_var, style="Dark.TEntry", width=20).grid(row=2, column=1, sticky="w", pady=4)

            # Requestor
            tk.Label(frm, text="Requestor:", bg=BACKGROUND_GRAY, fg=TEXT_DARK, font=FONT).grid(row=3, column=0, sticky="w")
            req_var = tk.StringVar(value=requestor or "")
            ttk.Entry(frm, textvariable=req_var, style="Dark.TEntry", width=60).grid(row=3, column=1, sticky="ew", pady=4)

            # PO
            tk.Label(frm, text="PO Number:", bg=BACKGROUND_GRAY, fg=TEXT_DARK, font=FONT).grid(row=4, column=0, sticky="w")
            po_var = tk.StringVar(value=po_number or "")
            ttk.Entry(frm, textvariable=po_var, style="Dark.TEntry", width=30).grid(row=4, column=1, sticky="w", pady=4)

            # ETD
            tk.Label(frm, text="ETD:", bg=BACKGROUND_GRAY, fg=TEXT_DARK, font=FONT).grid(row=5, column=0, sticky="w")
            etd_var_local = tk.StringVar(value=etd or "")
            cal = DateEntry(frm, selectmode='day', textvariable=etd_var_local, date_pattern='yyyy-mm-dd')
            cal.grid(row=5, column=1, sticky="w", pady=4)

            # Info: date requested (readonly)
            tk.Label(frm, text=f"Requested Date: {date_requested or ''}", bg=BACKGROUND_GRAY, fg=TEXT_GRAY, font=SMALL_FONT).grid(row=6, column=0, columnspan=2, sticky="w", pady=(6,0))
            ...
            def on_save():
                ...
                new_req = req_var.get().strip()
                new_po = po_var.get().strip()
                new_etd = etd_var_local.get().strip()
                ...
                # define status based on remaining
                if new_quantity_remaining <= 0:
                    new_status = 'fulfilled'
                    date_closed_val = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    new_quantity_remaining = 0
                elif new_quantity_remaining < new_qty:
                    new_status = 'partial'
                    date_closed_val = None
                else:
                    new_status = 'open'
                    date_closed_val = None
                ...
                try:
                    ...
                    c.execute("""
                        ...
                        SET material_id = ?, purpose = ?, quantity = ?, quantity_remaining = ?,
                            requestor = ?, status = ?, po_number = ?, estimated_delivery_date = ?, date_closed = ?
                        WHERE id = ?
                    """, (new_mid, new_purpose, new_qty, new_quantity_remaining, new_req, new_status, new_po, new_etd, date_closed_val, pr_id))
                    conn.commit()
                    conn.close()
                except Exception as e:
            ...
            ttk.Button(btns, text="Save", style="Dark.TButton", command=on_save).pack(side="left", padx=6)
            ttk.Button(btns, text="Cancel", style="Dark.TButton", command=lambda: (dlg.grab_release(), dlg.destroy())).pack(side="left", padx=6)
