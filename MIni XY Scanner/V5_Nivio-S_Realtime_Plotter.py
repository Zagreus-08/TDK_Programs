...
def read_loop():
    ...
    while True:
        ...
        if x0 == 0 and y0 == 0:
            ...
            y_max = 50  # Start small, will grow to 180 max
            x_range = 50  # Start small, will grow to 180 max
        ...
        # Let X and Y grow to match actual scan area (no artificial cap)
        if scan_active and frozen_x_range is None:
            # Update X range independently - use actual data
            if x0 > 0:
                if x0 > x_range:
                    x_range = x0

            # Update Y range independently - use actual data
            if y0 > 0:
                if y0 > y_max:
                    y_max = y0
            
            # Force square: both axes must match the larger value
            max_range = max(x_range, y_max)
            x_range = y_max = max_range
            
            # At 90% through scan, freeze at current detected maximum
            if data_cnt == 1900:
                frozen_x_range = max_range
                frozen_y_max = max_range
                x_range = max_range
                y_max = max_range
                print(f"[INFO] *** AUTO-FREEZE at 90%: Display locked at {max_range:.2f}x{max_range:.2f}mm ***")
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            ...
            # FREEZE the display range at current detected maximum to prevent expansion
            max_range = max(x_range, y_max)
            frozen_x_range = max_range
            frozen_y_max = max_range
            x_range = max_range
            y_max = max_range
            print(f"[INFO] *** Display range FROZEN at: {max_range:.2f}x{max_range:.2f}mm ***")
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    if frozen_x_range is not None and frozen_y_max is not None:
        # Scan has completed - use frozen values (actual scan area)
...
def show_loaded(xs, ys, zs):
    ...
    # Auto-detect X and Y axis maximums from loaded data (no cap - use actual data range)
    detected_x_max = float(np.max(xs)) if len(xs) > 0 else 100
    detected_y_max = float(np.max(ys)) if len(ys) > 0 else 100

    # Use actual detected range (no artificial cap)
    # Force square display: use the larger of the two dimensions for both axes
    max_dimension = max(detected_x_max, detected_y_max)
    x_range = max_dimension
    y_max = max_dimension

    print(f"[INFO] Detected X-max: {detected_x_max:.2f}mm, Y-max: {detected_y_max:.2f}mm")
    print(f"[INFO] Display set to: {x_range:.2f}x{y_max:.2f}mm (square)")

    # NO FILTERING - show ALL data from the scan
    # The scanner already limits what it sends based on calibrated scan area
    
    # Scale X and Y coordinates to fit in 0-100 display range (actual_range → 100x100)
    # Scanner sends: (0,0) at top-right, X increases left, Y increases down
    # Display wants: (0,0) at top-left, X increases right, Y increases down
    # Invert X axis (right to left) AND invert Y axis (bottom to top)
