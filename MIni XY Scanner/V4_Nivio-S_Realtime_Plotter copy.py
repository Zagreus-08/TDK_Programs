...
def read_loop():
    ...
    while True:
        ...
        if scan_active and frozen_x_range is None:
            # Update X range independently but cap at 180
            if x0 > 0 and x0 <= 180:
                if x0 > x_range:
                    x_range = min(x0, 180)

            # Update Y range independently but cap at 180
            if y0 > 0 and y0 <= 180:
                if y0 > y_max:
                    y_max = min(y0, 180)
            
            # Force square: both axes must match the larger value, but max 180
            ...
            if data_cnt == 1900:
                ...
                x_range = 180
                y_max = 180
                print(f"[INFO] *** AUTO-FREEZE at 90%: Display locked at 180x180mm ***")
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            ...
            x_range = 180
            y_max = 180
            print(f"[INFO] *** Display range FROZEN at: 180x180mm ***")
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    # Use the display ranges (frozen or current) directly - always 180x180
    label_x_range = 180
    label_y_max = 180
...
def show_loaded(xs, ys, zs):
    ...
    print(f"[INFO] Display set to: 180x180mm (square)")

    # Filter and scale data based on 180x180 range
    # Only show data where X <= 180 and Y <= 180
    mask = (xs <= 180) & (ys <= 180)
    xs = xs[mask]
    ys = ys[mask]
    zs = zs[mask]
    ...
    # Display wants: (0,0) at top-left, X increases right, Y increases down
    # Invert X axis (right to left) AND invert Y axis (bottom to top)
    xs = 100 - (xs * 100 / 180)  # Invert X: right becomes left
    ys = 100 - (ys * 100 / 180)  # Invert Y: bottom becomes top
