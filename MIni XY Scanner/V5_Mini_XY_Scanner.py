...
class SimpleScanClass:
    def __init__(self, xymove=None, row_step=None, x_speed=None, y_speed=None, gui=None, x_count=None, y_count=None):
        ...
        # This matches Phase 1 which uses y_total_mm (usable Y range) / Y Scan Count
        ...
        try:
            while True:
                ...
                if collect_data and (now - last_data_time >= data_interval):
                    last_data_time = now
                    try:
                        # Update position to get current Y
                        self.xy.update_position_y(direction_sign)
                        current_y_mm = self.xy.get_position_y_mm()
    ...
    def simple_scan(self):
        ...
        PHASE 2 - VERTICAL SCANNING (INVERTED):
          - Y scan count = number of COLUMNS (how many times to move LEFT)
          - X scan count = number of ROWS (how many times to move DOWN within each column)
          - Step sizes: Area X / Y count (for columns), Area Y / X count (for rows)
        """
        ...
        # Repeat for each Y-count: move down, then perform alternating horizontal pass
        ...
        # Return to home position (0,0) before Phase 2
        if SystemFuncClass.stop_flag:
            return
    
        print("\n=== RETURNING TO HOME BEFORE PHASE 2 ===")
        
        # Return X to home (right side) - move to 0mm
        print("Moving X to home (RIGHT to 0 mm)")
        ok = self.scan_x_to_position_mm_corrected(right_target_mm, collect_data=False, current_y_mm=current_y_mm)
        if not ok:
            print("simple_scan: return X to home aborted.")
            return
    
        # Return Y to home (top side) - move to 0mm
        print("Moving Y to home (UP to 0 mm)")
        ok = self.scan_y_to_position_mm_corrected(top_target_mm, collect_data=False, current_x_mm=0.0)
        if not ok:
            print("simple_scan: return Y to home aborted.")
            return
    
        print("Returned to home position (0,0)")
        
        # Add delay to let motors settle before Phase 2
        ...
        for count in range(1, self.y_count + 1):
            ...
            target_y_mm = top_target_mm if next_direction_y == "UP" else bottom_target_mm
            print(f"Column {count}: vertical pass {next_direction_y} to {target_y_mm:.2f} mm (collecting data)")
