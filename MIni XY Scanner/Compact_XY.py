...
PULSES_PER_MM_BASE = PULSES_PER_REV / DISTANCE_PER_REV_MM # 800 / 40 = 20.0

# CALIBRATION FACTORS: Actual distance / Measured distance
# X-axis: Physical machine moves 1000mm, time-based shows ~200mm → factor = 5.0
# Y-axis: Physical machine moves 1000mm, time-based shows ~400mm → factor = 2.5
CALIBRATION_FACTOR_X = 5.0
CALIBRATION_FACTOR_Y = 2.5

# Apply calibration factors to get actual PULSES_PER_MM for each axis
PULSES_PER_MM_X = PULSES_PER_MM_BASE / CALIBRATION_FACTOR_X  # 20.0 / 5.0 = 4.0
PULSES_PER_MM_Y = PULSES_PER_MM_BASE / CALIBRATION_FACTOR_Y  # 20.0 / 2.5 = 8.0

# For compatibility with existing code that uses PULSES_PER_MM
PULSES_PER_MM = PULSES_PER_MM_X  # Default to X-axis value
...
def mm_to_pulses_x(mm: float) -> int:
    return int(round(mm * PULSES_PER_MM_X))

def pulses_to_mm_x(pulses: int) -> float:
    return pulses / PULSES_PER_MM_X

def mm_to_pulses_y(mm: float) -> int:
    return int(round(mm * PULSES_PER_MM_Y))

def pulses_to_mm_y(pulses: int) -> float:
    return pulses / PULSES_PER_MM_Y

# -----------------------
# Serial Data Transmission
# -----------------------
def send_serial_metadata(scan_area_x_mm, scan_area_y_mm, x_count, y_count):
    """Send scan metadata to plotter via serial
    
    Args:
        scan_area_x_mm: Scan area X dimension in mm
        scan_area_y_mm: Scan area Y dimension in mm
        x_count: Number of X scan points
        y_count: Number of Y scan points
    """
    global serial_port
    if serial_port and serial_port.is_open:
        try:
            # Format: METADATA,area_x,area_y,count_x,count_y\n
            metadata_str = f"METADATA,{scan_area_x_mm:.2f},{scan_area_y_mm:.2f},{x_count},{y_count}\n"
            serial_port.write(metadata_str.encode('ascii'))
            serial_port.flush()
            print(f"Sent metadata: {scan_area_x_mm}x{scan_area_y_mm}mm, X={x_count}, Y={y_count}")
        except Exception as e:
            print(f"Serial metadata send error: {e}")
...
class XYMoveClass(MotorClass, PortDefineClass, SystemFuncClass):
    ...
    def __init__(self, gui_config=None):
        # Store reference to GUI config for accessing calibrated dimensions
        ...
        # Calibration multiplier to compensate for missed pulses in software counting
        # Software callbacks miss ~80% of pulses at high speeds (800 Hz PWM)
        # FIXED multiplier: 1000mm actual / 200mm measured = 5.0
        # This is ALWAYS applied to all position tracking
        self._pulse_calibration_multiplier = 5.0  # Fixed: compensates for ~20% pulse capture rate
    ...
    def reset_position_counters(self):
        ...
        # Reset time tracking for time-based position calculation
        self._last_x_update_time = time.time()
        self._last_y_update_time = time.time()
    
    def set_pulse_calibration_multiplier(self, multiplier):
        """Set the calibration multiplier to compensate for missed pulses."""
        self._pulse_calibration_multiplier = float(multiplier)
        print(f"Pulse calibration multiplier set to: {self._pulse_calibration_multiplier:.3f}")

    def get_position_x_mm(self):
        """Get current X position in mm with limit switch correction."""
        # Auto-correct position when at limits
        try:
            if self.CheckXlimit_neg():
                # At X- limit, position should be at -area_x_mm
                cfg = getattr(self, '_gui_config', {})
                if not cfg and hasattr(self, '_parent_gui'):
                    cfg = getattr(self._parent_gui, 'saved_config', {})
                area_x = cfg.get("area_x_mm", 1000.0) if cfg else 1000.0
                self._position_x_mm = -area_x
            elif self.CheckXlimit_pos():
                # At X+ limit (home), position should be 0
                self._position_x_mm = 0.0
        except:
            pass
    ...
    def get_position_y_mm(self):
        """Get current Y position in mm with limit switch correction."""
        # Auto-correct position when at limits
        try:
            if self.CheckYlimit_neg():
                # At Y- limit, position should be at -area_y_mm
                cfg = getattr(self, '_gui_config', {})
                if not cfg and hasattr(self, '_parent_gui'):
                    cfg = getattr(self._parent_gui, 'saved_config', {})
                area_y = cfg.get("area_y_mm", 1000.0) if cfg else 1000.0
                self._position_y_mm = -area_y
            elif self.CheckYlimit_pos():
                # At Y+ limit (home), position should be 0
                self._position_y_mm = 0.0
        except:
            pass
    ...
    def update_position_x(self, direction_sign):
        """
        Update X position based on TIME and SPEED instead of pulse counting.
        This is more reliable than software pulse counting at high speeds.
        direction_sign: +1 for RIGHT, -1 for LEFT
        """
        try:
            # Get the current PWM frequency (speed in Hz = pulses per second)
            current_speed_hz = pi.get_PWM_frequency(PortDefineClass.STEP1)
            
            # Calculate time elapsed since last update
            current_time = time.time()
            if not hasattr(self, '_last_x_update_time'):
                self._last_x_update_time = current_time
                return
            
            time_elapsed = current_time - self._last_x_update_time
            self._last_x_update_time = current_time
            
            # Calculate distance moved: speed (pulses/sec) × time (sec) = pulses
            # Then convert pulses to mm using X-axis calibration
            pulses_moved = current_speed_hz * time_elapsed
            distance_mm = pulses_to_mm_x(pulses_moved) * direction_sign
            
            self._position_x_mm += distance_mm
        except Exception as e:
            print(f"update_position_x error: {e}")

    def update_position_y(self, direction_sign):
        """
        Update Y position based on TIME and SPEED instead of pulse counting.
        This is more reliable than software pulse counting at high speeds.
        ...
        try:
            ...
            # Calculate time elapsed since last update
            current_time = time.time()
            if not hasattr(self, '_last_y_update_time'):
                self._last_y_update_time = current_time
                return
            
            time_elapsed = current_time - self._last_y_update_time
            self._last_y_update_time = current_time
            
            # Calculate distance moved: speed (pulses/sec) × time (sec) = pulses
            # Then convert pulses to mm using Y-axis calibration
            pulses_moved = current_speed_hz * time_elapsed
            distance_mm = pulses_to_mm_y(pulses_moved) * direction_sign
            
            self._position_y_mm += distance_mm
        except Exception as e:
            print(f"update_position_y error: {e}")
...
class SimpleScanClass:
    ...
    def simple_scan(self):
        ...
        PHASE 1 - HORIZONTAL SCANNING:
          ...
          - Scans full width (LEFT-RIGHT) for each row
          - Step size: Area Y / Y count
        ...
        # Send metadata to plotter (scan area and counts)
        # Get scan area from config (calibrated dimensions)
        try:
            cfg = self.gui.saved_config if hasattr(self, 'gui') and self.gui else {}
            area_x_mm = cfg.get("area_x_mm", 920.0)  # Default to 920 if not calibrated
            area_y_mm = cfg.get("area_y_mm", 920.0)
            send_serial_metadata(area_x_mm, area_y_mm, self.x_count, self.y_count)
            time.sleep(0.1)
        except Exception as e:
        ...
        # ========== PHASE 1: HORIZONTAL SCANNING ==========
        print("\n=== PHASE 1: HORIZONTAL SCANNING ===")
        print(f"Scanning {self.y_count} horizontal rows")
        
        current_y_mm = 0.0
    
        # Start with an initial LEFT scan from home (X=0, Y=0, top-right corner)
        print("Row 0: LEFT scan from top-right (0,0) (collecting data)")
        ...
        # Repeat for each Y-count: move down, then perform alternating horizontal p/ass
        for count in range(1, self.y_count + 1):
            ...
            print(f"Row {count}: horizontal pass {next_direction} to {target_x_mm:.2f} mm (collecting data)")
    
            ok = self.scan_x_to_position_mm_corrected(target_x_mm, collect_data=True, current_y_mm=current_y_mm)
            if not ok:
                print(f"simple_scan: horizontal move aborted (limit/EMG/stop).")
                break
...
class DataScanClass:
    def __init__(self, gui=None):
        self.gui = gui
        # Pass GUI's saved_config to XYMoveClass for accessing calibrated dimensions
        gui_config = gui.saved_config if gui and hasattr(gui, 'saved_config') else None
        self.xymove = XYMoveClass(gui_config=gui_config)
...
class GUIClass(PortDefineClass):
    ...
    def __init__(self):
        ...
        # Load and apply calibration multiplier from config (default 5.0 if not set)
        try:
            multiplier = self.saved_config.get("pulse_calibration_multiplier", 5.0)
            self.data_scan.xymove.set_pulse_calibration_multiplier(multiplier)
            print(f"Loaded pulse calibration multiplier from config: {multiplier}")
        except Exception as e:
            print(f"Failed to load calibration multiplier, using default (5.0): {e}")
    ...
    def stop_all_motion(self):
        ...
        print("!!! STOP BUTTON PRESSED - EMERGENCY STOP !!!")
        
        # Set stop flag FIRST (highest priority)
        SystemFuncClass.stop_flag = True
        
        # Stop all PWM immediately (hardware level)
        try:
            pi.set_PWM_dutycycle(PortDefineClass.STEP1, 0)
            pi.set_PWM_dutycycle(PortDefineClass.STEP2, 0)
            print("PWM stopped on STEP1 and STEP2")
        except Exception as e:
        ...
        try:
            self._jog_active_x = False
            self._jog_active_y = False
            print("Jog threads signaled to stop")
        except Exception:
            pass
        ...
        try:
            if self.data_scan and getattr(self.data_scan, 'xymove', None):
                try:
                    ...
                    print("XYMove motors stopped")
                except Exception as e:
                    print(f"Error stopping XYMove: {e}")
        except Exception:
            pass
        
        # Call AllStop for any remaining cleanup
        try:
            SystemFuncClass().AllStop()
        except Exception:
            pass

        # Indicate stopped in UI (red)
        self.set_status("STOPPED", bg='red', blink=False)
        
        # Reset stop flag after a short delay (in background thread to not block UI)
        def _reset_stop_flag():
            ...
            if not self._emg_active:
                self.set_status("READY", bg=self.status_ready_bg, blink=False)
            print("Stop flag cleared, system ready")
        
        threading.Thread(target=_reset_stop_flag, daemon=True).start()
    ...
    def _area_calibration_worker(self):
        ...
        try:
            ...
            try:
                ...
                # Update UI to show 0,0 immediately after reset
                self.win.after(0, lambda: self.area_x_display.config(text=f"Area X (mm): 0"))
                self.win.after(0, lambda: self.area_y_display.config(text=f"Area Y (mm): 0"))
                print("Position counters reset to (0,0) for calibration")
            except Exception as e:
                print(f"Failed to reset position counters: {e}")
                    ...
                    if not aborted:
                        ...
                        try:
                            ...
                            while True:
                                ...
                                if xy.CheckXlimit_neg():
                                    ...
                                    try:
                                        ...
                                        pass
                                    # CRITICAL: Update position one final time before reading extent
                                    try:
                                ...
                                # Periodically update realtime X display AND position tracking
                                now = time.time()
                                if now - last_ui >= 0.05:  # Update every 50ms for accuracy
                                    last_ui = now
                                    try:
                                        xy.update_position_x(direction_sign)
                                ...
                                sleep(0.001)  # Poll frequently for limit detection
                        finally:
                            ...
                            try:
                                ...
                                pass
                            # CRITICAL: Final position update to capture any remaining pulses
                            try:
                                xy.update_position_x(direction_sign)
                            except Exception:
                                pass
                        ...
                        if not aborted:
                            ...
                            try:
                                ...
                                while True:
                                    ...
                                    # Periodic UI updates - increased frequency for accuracy
                                    now = time.time()
                                    if now - last_ui >= 0.05:  # Update every 50ms for accuracy
                                        last_ui = now
                                        try:
            ...
            # Save the measured extents (already corrected by the 5.0 multiplier)
            # These should now be close to 1000x1000mm
            if not aborted and (x_extent > 0 or y_extent > 0):
                try:
                    cfg = dict(self.saved_config or {})
                    cfg["area_x_mm"] = float(round(x_extent, 3))
                    cfg["area_y_mm"] = float(round(y_extent, 3))
                    # Ensure multiplier is saved
                    if "pulse_calibration_multiplier" not in cfg:
                        cfg["pulse_calibration_multiplier"] = 5.0
                    save_config(cfg)
                    self.saved_config = cfg
            ...
            if aborted:
                ...
                try:
                    area_x = self.saved_config.get("area_x_mm", 0) if self.saved_config else 0
                    area_y = self.saved_config.get("area_y_mm", 0) if self.saved_config else 0
                    
                    msg = f"Calibration Complete!\n\n"
                    msg += f"Measured Scan Area:\n{int(round(area_x))} × {int(round(area_y))} mm\n\n"
                    msg += f"Position tracking is calibrated."
                    self.win.after(0, lambda: messagebox.showinfo("Calibration Complete", msg))
                except Exception:
                    pass
