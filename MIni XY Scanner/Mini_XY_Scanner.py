...
class SimpleScanClass:
    ...
    def scan_x_to_position_mm(self, target_mm, hold_after_limit=0.05):
        """
    ...
    def simple_scan(self):
        ...
        print("Initial horizontal pass: LEFT")
        ok = self.scan_x_to_position_mm(left_target_mm)
        if not ok:
        ...
        for count in range(1, self.y_count + 1):
            ...
            if not moved:
                ...
                try:
                    if last_direction == "LEFT":
                        print("Final sweep after Y-limit: RIGHT")
                        self.scan_x_to_position_mm(right_target_mm)
                    else:
                        print("Final sweep after Y-limit: LEFT")
                        self.scan_x_to_position_mm(left_target_mm)
                except Exception:
                    pass
                break
            ...
            print(f"Count {count}/{self.y_count}: horizontal pass {next_direction} to {target_x_mm} mm")
    
            ok = self.scan_x_to_position_mm(target_x_mm)
            if not ok:
