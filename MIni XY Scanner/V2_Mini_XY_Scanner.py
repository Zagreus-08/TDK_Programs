...
import serial
import random  # For simulated sensor data (replace with actual sensor reading)
...
def read_sensor_value():
    """
    Read actual sensor value here.
    Replace this with your actual sensor reading code.
    For now, returns simulated data.
    """
    # TODO: Replace with actual sensor reading (ADC, analog input, etc.)
    # Example: return adc.read_voltage() or similar
    return random.uniform(-0.1, 0.1)  # Simulated sensor data
...
class SimpleScanClass:
    ...
    def simple_scan(self):
        ...
        Sequence:
          - Send start marker (0,0,0) to plotter
          - Start with an initial LEFT scan from Home (assumed X == 0).
          - For i in 1..Y_count:
        ...
        left_target_mm = -self.x_total_mm
        right_target_mm = 0.0
        ...
        print(f"Scan config: left_target_mm={left_target_mm} mm, y_step_mm={y_step_mm} mm, y_count={self.y_count}")
    
        # Send start marker to plotter (0,0,0)
        send_serial_data(0.0, 0.0, 0.0)
        print("Sent start marker (0,0,0) to plotter")
        time.sleep(0.1)  # Small delay to ensure plotter receives marker
    
        # Track current Y position for data collection
        current_y_mm = 0.0
    
        # Start with an initial LEFT scan from home (X == 0)
        print("Initial horizontal pass: LEFT (collecting data)")
        ok = self.scan_x_to_position_mm(left_target_mm, collect_data=True, current_y_mm=current_y_mm)
        if not ok:
        ...
        for count in range(1, self.y_count + 1):
            ...
            if not moved:
                ...
                try:
                    if last_direction == "LEFT":
                        ...
                        self.scan_x_to_position_mm(right_target_mm, collect_data=True, current_y_mm=abs(self.xy.get_position_y_mm()))
                    else:
                        print("Final sweep after Y-limit: LEFT (collecting data)")
                        self.scan_x_to_position_mm(left_target_mm, collect_data=True, current_y_mm=abs(self.xy.get_position_y_mm()))
                except Exception:
                    pass
                break
            ...
            ok = self.scan_x_to_position_mm(target_x_mm, collect_data=True, current_y_mm=current_y_mm)
            if not ok:
