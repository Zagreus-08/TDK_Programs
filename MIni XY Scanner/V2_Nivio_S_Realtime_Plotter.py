...
# -*- coding: utf-8 -*-
"""
Nivio-S Realtime Plotter - Standalone Version with MCC DAQ HAT
Ver 1.0   2025-01-XX

FEATURES:
- Receives X,Y coordinates via serial from Mini XY Scanner
- Reads Z and Z2 sensor data directly from MCC 128 DAQ HAT
- Saves complete raw CSV data: x, y, z, z2 (4 columns)
- Real-time 2D/3D plotting with auto-scaling
...
ARCHITECTURE:
- V10_Mini_XY_Scanner: Sends only X,Y positions via serial
- V1_Nivio_S_Plotter (this program): 
  * Receives X,Y from serial
  * Reads Z,Z2 from local MCC 128 DAQ HAT
  * Combines and displays/saves data
"""
...
from daqhats import mcc128, HatIDs, OptionFlags
mcc128_hat = mcc128(0)
mcc128_channels = [0, 1]  # Both channels 0 and 1 for Z and Z2
mcc128_samples_per_channel = 10  # Small buffer for low latency
mcc128_scan_rate = 1000  # 1000 Hz sampling rate
mcc128_options = OptionFlags.CONTINUOUS  # CONTINUOUS streaming mode
mcc128_timeout = 1.0

# Latest sensor values (continuously updated by background thread)
latest_z_value = 0.0
latest_z2_value = 0.0
sensor_lock = threading.Lock()  # Thread-safe access to sensor values
...
# ---------------- CONTINUOUS Sensor Streaming Thread ----------------
def continuous_sensor_reader():
    """
    BACKGROUND THREAD: Continuously reads sensor data and updates latest values.
    This runs independently of position updates for TRUE real-time synchronization.
    
    When position data arrives via serial, it immediately grabs the LATEST sensor value
    that was captured in parallel, achieving near-zero lag between position and detection.
    """
    global mcc128_hat, latest_z_value, latest_z2_value, sensor_lock, sensor_thread_running
    
    if mcc128_hat is None:
        print("[WARNING] No MCC 128 DAQ HAT - sensor thread not started")
        return
    
    print("[INFO] Starting CONTINUOUS sensor streaming thread...")
    sensor_thread_running = True
    
    try:
        channel_mask = chan_list_to_mask(mcc128_channels)
        samples_per_read = mcc128_samples_per_channel * len(mcc128_channels)
        
        # Start continuous scan
        mcc128_hat.a_in_scan_start(channel_mask, mcc128_samples_per_channel, mcc128_scan_rate, mcc128_options)
        print("[INFO] ✓ Sensor streaming active - real-time detection enabled")
        
        while sensor_thread_running:
            try:
                # Read latest samples (non-blocking with short timeout)
                read_result = mcc128_hat.a_in_scan_read(samples_per_read, 0.1)
                
                if read_result.data and len(read_result.data) > 0:
                    # Data is interleaved: [ch0, ch1, ch0, ch1, ...]
                    # Get the MOST RECENT samples (last few readings)
                    recent_samples = read_result.data[-20:]  # Last 20 samples (10 per channel)
                    
                    ch0_data = [recent_samples[i] for i in range(0, len(recent_samples), 2)]
                    ch1_data = [recent_samples[i] for i in range(1, len(recent_samples), 2)]
                    
                    # Calculate moving average for noise reduction
                    z_avg = sum(ch0_data) / len(ch0_data) if ch0_data else 0.0
                    z2_avg = sum(ch1_data) / len(ch1_data) if ch1_data else 0.0
                    
                    # Update global latest values (thread-safe)
                    with sensor_lock:
                        latest_z_value = z_avg
                        latest_z2_value = z2_avg
                        
            except Exception as e:
                if sensor_thread_running:  # Only log if not intentionally stopped
                    print(f"[WARNING] Sensor read error: {e}")
                time.sleep(0.01)  # Brief pause before retry
                
    except Exception as e:
        print(f"[ERROR] Sensor streaming thread error: {e}")
    finally:
        # Cleanup when thread stops
        try:
            mcc128_hat.a_in_scan_stop()
            mcc128_hat.a_in_scan_cleanup()
            print("[INFO] Sensor streaming stopped")
        except:
            pass
...
def get_latest_sensor_values():
    """
    Instantly retrieves the LATEST sensor values captured by background thread.
    This is called when X,Y position data arrives - NO WAITING, just grab current values.
    
    Returns:
        tuple: (z_voltage, z2_voltage) - the most recent sensor readings
    """
    global latest_z_value, latest_z2_value, sensor_lock
    
    with sensor_lock:
        return (latest_z_value, latest_z2_value)
...
def read_loop():
    ...
    while True:
        ...
        try:
            ...
            # NEW FORMAT: Parse X,Y only from scanner (no Z values sent)
            # Scanner now sends: x,y (2 fields only)
            parts = line.split(",")
            if len(parts) < 2:
                print(f"data error @count={data_cnt}: insufficient fields (need at least X,Y)")
                continue
            ...
            # INSTANT GRAB: Get the LATEST sensor values from continuous background stream
            # NO DELAY - values are already being captured in parallel by sensor thread
...
if __name__ == '__main__':
    # Start continuous sensor streaming thread FIRST
    sensor_stream_thread = threading.Thread(target=continuous_sensor_reader, daemon=True)
    sensor_stream_thread.start()
    
    # Wait briefly to ensure sensor thread is running
    time.sleep(0.5)
    
    # Then start serial communication thread
    ...
    def do_exit():
        ...
        if messagebox.askyesno("Exit", "Close the program?"):
            # Stop sensor streaming thread
            sensor_thread_running = False
            time.sleep(0.2)  # Allow thread to cleanup
