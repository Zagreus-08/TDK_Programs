...
def read_loop():
    ...
    while True:
        ...
        if x0 == 0 and y0 == 0:
            ...
            y_max = 50  # Start small, will grow to 920 max
            x_range = 50  # Start small, will grow to 920 max
        ...
        # Let X and Y grow based on actual scan data, but use metadata if available
        if scan_active and frozen_x_range is None:
            # Use metadata dimensions if available, otherwise grow with data
            if scan_area_x > 0 and scan_area_y > 0:
                # Use the calibrated area dimensions from scanner
                x_range = int(scan_area_x)
                y_max = int(scan_area_y)
                print(f"[INFO] Using scan area from metadata: {x_range}x{y_max}mm")
            else:
                # Fallback: grow with data but cap at 920
                if x0 > 0 and x0 <= 920:
                    if x0 > x_range:
                        x_range = min(x0, 920)
                ...
                # Force square: both axes must match the larger value, but max 920
                max_range = min(max(x_range, y_max), 920)
                x_range = y_max = max_range
            
            # At 90% through scan, freeze the display range
            if data_cnt == 1900:
                frozen_x_range = x_range
                frozen_y_max = y_max
                print(f"[INFO] *** AUTO-FREEZE at 90%: Display locked at {x_range}x{y_max}mm ***")
        ...
        # ---------- End of scan ----------
        # Detect end of scan: hardware sends matching max values
        # Use the actual scan dimensions (x_range, y_max) for detection
        # Only trigger if we have enough data points (avoid false positives)
        if scan_active and data_cnt > 50:
            # Check if we've reached the expected scan area limits
            # Allow 5mm tolerance for detection
            at_x_limit = abs(x0 - x_range) <= 5
            at_y_limit = abs(y0 - y_max) <= 5
            
            if at_x_limit and at_y_limit:
                ...
                # FREEZE the display range to prevent expansion
                frozen_x_range = x_range
                frozen_y_max = y_max
                print(f"[INFO] *** Display range FROZEN at: {x_range}x{y_max}mm ***")
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    # FIXED: Lock zmin/zmax after first plot to prevent color changes
    # Once we have data, freeze the color scale so early plots don't change color
    if not z_range_locked:
        # Only expand range on FIRST update (when zmin/zmax are still at defaults)
        # After that, keep the range fixed so colors don't shift
    ...
    # Use the actual display ranges (frozen or current)
...
def show_loaded(xs, ys, zs):
    ...
    print(f"[INFO] Display set to: 920x920mm (square)")

    # Filter and scale data based on 920x920 range
    # Only show data where X <= 920 and Y <= 920
    mask = (xs <= 920) & (ys <= 920)
    xs = xs[mask]
    ys = ys[mask]
    zs = zs[mask]
    ...
    # Display wants: (0,0) at top-left, X increases right, Y increases down
    # Invert X axis (right to left) AND invert Y axis (bottom to top)
    xs = 100 - (xs * 100 / 920)  # Invert X: right becomes left
    ys = 100 - (ys * 100 / 920)  # Invert Y: bottom becomes top
    ...
    if len(unique_xs) > 250:
        unique_xs = np.linspace(unique_xs.min(), unique_xs.max(), 250)
    if len(unique_ys) > 250:
        unique_ys = np.linspace(unique_ys.min(), unique_ys.max(), 250)
