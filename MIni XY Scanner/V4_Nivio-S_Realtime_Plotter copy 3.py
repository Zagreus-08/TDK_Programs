...
def read_loop():
    ...
    while True:
        ...
        if scan_active and frozen_x_range is None:
            # Update X range independently but cap at 920
            if x0 > 0 and x0 <= 920:
                if x0 > x_range:
                    x_range = min(x0, 920)

            # Update Y range independently but cap at 920
            if y0 > 0 and y0 <= 920:
                if y0 > y_max:
                    y_max = min(y0, 920)
            ...
            if data_cnt == 1900:
                ...
                x_range = 920
                y_max = 920
                print(f"[INFO] *** AUTO-FREEZE at 90%: Display locked at 920x920mm ***")
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            ...
            x_range = 920
            y_max = 920
            print(f"[INFO] *** Display range FROZEN at: 920x920mm ***")
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    # Use the display ranges (frozen or current) directly - always 920x920
    label_x_range = 920
    label_y_max = 920
