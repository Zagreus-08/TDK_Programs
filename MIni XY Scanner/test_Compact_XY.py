...
class SimpleScanClass:
    ...
    def simple_scan(self):
        ...
        # Calculate targets
        left_target_mm = 190.0  # Fixed target: scan until X=180mm in realtime position
        ...
        print(f"Pattern: DOWN to Y=190mm (data) -> UP to Y=0mm (no data) -> LEFT -> Repeat")
        print(f"Column step: {self.column_step} pulses (~{phase2_column_step_mm:.2f} mm)")
        
        current_x_mm = 0.0  # Start from home position (0,0)
    
        # Column 0: Initial DOWN scan from home (X=0, Y=0, top-right corner) with data collection
        print("Column 0: DOWN scan from (0,0) to Y=190mm (collecting data)")
        ...
        # Return UP without data collection
        print("Column 0: UP return to Y=0mm (no data collection)")
        ok = self.scan_up_until_threshold(threshold_mm=5.0, current_x_mm=current_x_mm, collect_data=False)
        if not ok:
        ...
        for count in range(1, self.y_count + 1):
            ...
            # Scan DOWN with data collection
            print(f"Column {count}: DOWN scan to Y=190mm (collecting data)")
            ok = self.scan_y_to_position_mm_corrected(bottom_target_mm, collect_data=True, current_x_mm=current_x_mm)
            if not ok:
            ...
            # Return UP without data collection
            print(f"Column {count}: UP return to Y=0mm (no data collection)")
            ok = self.scan_up_until_threshold(threshold_mm=5.0, current_x_mm=current_x_mm, collect_data=False)
            if not ok:
