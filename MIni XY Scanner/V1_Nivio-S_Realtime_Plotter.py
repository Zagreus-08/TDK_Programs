...
# Line buffering for optimized plotting
line_buffer_x = []
line_buffer_y = []
line_buffer_z = []
last_y_value = None
plot_update_needed = False
lines_completed = 0  # Counter for lines completed

# ============ PERFORMANCE TUNING PARAMETERS ============
ENABLE_LIVE_PLOTTING = False  # False = Phase 1 only (buffer), True = show during scan
UPDATE_EVERY_N_LINES = 5  # Update plot every N lines (only if ENABLE_LIVE_PLOTTING=True)
MAX_DISPLAY_POINTS = 5000  # Max points to display during live scan
ANIMATION_INTERVAL = 500  # Milliseconds between plot checks
# Data averaging for final plot (Phase 2)
AVERAGE_HORIZONTAL = True  # Average data points horizontally (X-axis)
AVERAGE_VERTICAL = True    # Average data points vertically (Y-axis)
AVERAGING_WINDOW = 3       # Window size for averaging (3 = average every 3 points)
# =======================================================

# Phase tracking
scan_phase = 1  # 1 = buffering data (Phase 1), 2 = processing and plotting (Phase 2)
current_scan_phase_name = "Idle"  # Track phase name from scanner: "PHASE1_HORIZONTAL", "PHASE2_VERTICAL", or "Idle"
...
# ---------------- Save figures ----------------
def average_data(xs, ys, zs, window=3):
    """
    Average data points to reduce noise and data density
    Args:
        xs, ys, zs: Input data arrays
        window: Averaging window size (e.g., 3 = average every 3 points)
    Returns:
        Averaged xs, ys, zs arrays
    """
    if window <= 1 or len(xs) < window:
        return xs, ys, zs
    
    print(f"[PHASE 2] Averaging data with window={window}...")
    print(f"[PHASE 2] Original data points: {len(xs)}")
    ...
    # Create a structured array to group by (x, y) coordinates
    from collections import defaultdict
    coord_dict = defaultdict(list)
    
    # Group Z values by their (X, Y) coordinates
    for i in range(len(xs_arr)):
        # Round coordinates to create bins for averaging
        x_bin = np.round(xs_arr[i] / window) * window
        y_bin = np.round(ys_arr[i] / window) * window
        coord_dict[(x_bin, y_bin)].append(zs_arr[i])
    
    # Average the Z values for each coordinate bin
    xs_avg = []
    ys_avg = []
    zs_avg = []
    
    for (x_bin, y_bin), z_values in coord_dict.items():
        xs_avg.append(x_bin)
        ys_avg.append(y_bin)
        zs_avg.append(np.mean(z_values))  # Average all Z values in this bin
    
    print(f"[PHASE 2] Averaged data points: {len(xs_avg)}")
    print(f"[PHASE 2] Data reduction: {100 * (1 - len(xs_avg)/len(xs)):.1f}%")
    
    return xs_avg, ys_avg, zs_avg
...
def read_loop():
    ...
    global line_buffer_x, line_buffer_y, line_buffer_z, last_y_value, plot_update_needed, lines_completed
    global scan_phase, current_scan_phase_name
    data_cnt = 0
    ...
    while True:
        ...
        try:
            ...
            # Check for phase status message (PHASE,phase_name)
            if line.startswith("PHASE,"):
                parts = line.split(",")
                if len(parts) >= 2:
                    phase_name = parts[1].strip()
                    current_scan_phase_name = phase_name
                    print(f"[SERIAL] Received phase status: {phase_name}")
                    # Update GUI label if it exists
                    try:
                        if phase_name == "PHASE1_HORIZONTAL":
                            phase_status_label.config(text="Scanning: Horizontal", fg="blue")
                        elif phase_name == "PHASE2_VERTICAL":
                            phase_status_label.config(text="Scanning: Vertical", fg="green")
                        else:
                            phase_status_label.config(text=f"Scanning: {phase_name}", fg="purple")
                    except NameError:
        ...
        if x0 == 0 and y0 == 0:
            print("[INFO] ========== NEW SCAN DETECTED ==========")
            print("[INFO] Resetting all buffers and starting Phase 1...")
            ...
            last_y_value = None
            plot_update_needed = False
            lines_completed = 0  # Reset line counter

            # CRITICAL: Reset to Phase 1 for new scan
            scan_phase = 1
            print("[PHASE 1] Buffering mode activated - NO plotting during scan")
        ...
        # ---------- LINE-BASED BUFFERING FOR PHASE 1 (NO PLOTTING) ----------
        # Detect when Y coordinate changes (new line starts)
        if last_y_value is None:
            last_y_value = y0
        
        # Check if we've moved to a new line (Y value changed)
        if y0 != last_y_value:
            # Flush the previous line's buffer to main arrays
            if line_buffer_x:
                x.extend(line_buffer_x)
                y.extend(line_buffer_y)
                z.extend(line_buffer_z)
                lines_completed += 1
                
                # PHASE 1: Only buffer data, NO plotting during scan
                # plot_update_needed will be set to True only at end of scan
                if ENABLE_LIVE_PLOTTING and lines_completed % UPDATE_EVERY_N_LINES == 0:
                    plot_update_needed = True
                    print(f"[PHASE 1] {lines_completed} lines buffered, plot update triggered")
                else:
                    print(f"[PHASE 1] {lines_completed} lines buffered (no plot yet)")
                
                # Clear line buffer for new line
                line_buffer_x.clear()
                line_buffer_y.clear()
                line_buffer_z.clear()
        ...
        # Add current point to line buffer
        line_buffer_x.append(x0)
        line_buffer_y.append(y0)
        line_buffer_z.append(z0)
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            # Flush any remaining buffered data before ending scan
            if line_buffer_x:
                x.extend(line_buffer_x)
                y.extend(line_buffer_y)
                z.extend(line_buffer_z)
                print(f"[PHASE 1] Final line flushed: {len(line_buffer_x)} points")
                line_buffer_x.clear()
                line_buffer_y.clear()
                line_buffer_z.clear()
            
            print(f"[INFO] ========== END OF SCAN DETECTED ==========")
            print(f"[PHASE 1] Scan complete at ({x0},{y0})")
            print(f"[PHASE 1] Total data points buffered: {len(x)}")
            print(f"[PHASE 1] Display range: {x_range}x{y_max}mm")
            
            # ========== TRANSITION TO PHASE 2 ==========
            print(f"[PHASE 2] Starting data processing and averaging...")
            scan_phase = 2  # CRITICAL: Transition to Phase 2
            
            # Apply data averaging if enabled
            if AVERAGE_HORIZONTAL or AVERAGE_VERTICAL:
                print(f"[PHASE 2] Averaging enabled (window={AVERAGING_WINDOW})")
                x_avg, y_avg, z_avg = average_data(x, y, z, window=AVERAGING_WINDOW)
                # Replace data with averaged data
                x.clear()
                y.clear()
                z.clear()
                x.extend(x_avg)
                y.extend(y_avg)
                z.extend(z_avg)
                print(f"[PHASE 2] Data averaging complete: {len(x)} points after averaging")
            else:
                print(f"[PHASE 2] No averaging applied")
            
            # FREEZE the display range at current values to prevent expansion
            frozen_x_range = x_range
            frozen_y_max = y_max
            print(f"[PHASE 2] Display range FROZEN at: {frozen_x_range}x{frozen_y_max}mm")
            
            # Trigger plot update for Phase 2
            plot_update_needed = True
            print(f"[PHASE 2] Plot update triggered - will display averaged data")
            ...
            try:
                if filename_from_serial:
                    ...
                    print(f"[PHASE 2] Attempting to save plot to: {save_path}")
            ...
            # Reset phase status label
            try:
                phase_status_label.config(text="Idle", fg="gray")
            except NameError:
                pass  # Label not created yet
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    global scan_area_x, scan_area_y, scan_count_x, scan_count_y, frozen_x_range, frozen_y_max
    global plot_update_needed, scan_phase
    
    # CRITICAL: PHASE 1 BLOCKING - NO PLOTTING during active scan
    # scan_phase == 1 means we're buffering data (Phase 1)
    # scan_phase == 2 means scan completed, ready to plot (Phase 2)
    if scan_phase == 1:
        # During Phase 1, NEVER plot - just buffer data
        return
    
    # If we're viewing loaded data (pause_live), allow plotting
    if pause_live:
        if len(x) < 2:
            return
        # Continue to plotting below
    else:
        # Phase 2: Only plot when explicitly triggered
        if not plot_update_needed:
            return
...
def resume_live():
    ...
    global frozen_x_range, frozen_y_max, line_buffer_x, line_buffer_y, line_buffer_z, last_y_value, plot_update_needed
    pause_live = False
...
if __name__ == '__main__':
    ...
    phase_status_frame = tk.Frame(controls_frame, bg="#d9d9d9", relief="sunken", bd=2)
    phase_status_frame.pack(pady=8, fill=tk.X)
    
    tk.Label(phase_status_frame, text="Scan Phase:", font=("Arial", 10, "bold"), 
             bg="#d9d9d9", fg="black").pack(anchor="w", padx=5, pady=(5, 0))
    
    phase_status_label = tk.Label(phase_status_frame, text="Idle", font=("Arial", 11, "bold"), 
                                   bg="#d9d9d9", fg="gray", wraplength=150, justify="left")
    phase_status_label.pack(anchor="w", padx=5, pady=(0, 5))
