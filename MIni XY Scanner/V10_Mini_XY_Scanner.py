...
def send_serial_data(x_mm, y_mm):
    """Send scan X,Y position to plotter via serial
    
    The plotter (V1_Nivio_S_Realtime_Plotter.py) will:
    - Receive X,Y coordinates
    - Read Z,Z2 from its own local MCC 128 DAQ HAT
    - Combine and save complete data
    ...
    if serial_port and serial_port.is_open:
        try:
            # NEW FORMAT: x,y\n (only 2 fields - no Z data sent)
            data_str = f"{x_mm:.2f},{y_mm:.2f}\n"
...
class SimpleScanClass:
            ...
            while True:
                ...
                if collect_data and (now - last_data_time >= data_interval):
                    ...
                    try:
                        ...
                        # Send X,Y to plotter (plotter will read its own Z,Z2)
                        abs_x_mm = abs(current_x_mm)
                        abs_y_mm = abs(current_y_mm)
                        send_serial_data(abs_x_mm, abs_y_mm)
                        
                        print(f"Sent position: X={abs_x_mm:.2f}, Y={abs_y_mm:.2f}")
                    except Exception as e:
            ...
            while True:
                ...
                if collect_data and (now - last_data_time >= data_interval):
                    ...
                    try:
                        ...
                        # Send X,Y position to plotter (plotter reads its own Z,Z2)
                        send_serial_data(corrected_x_mm, corrected_y_mm)
                        
                        print(f"Position: X={corrected_x_mm:.2f}mm ({progress_x:.1f}%), Y={corrected_y_mm:.2f}mm ({progress_y:.1f}%)")
                    except Exception as e:
                        print(f"Data collection error: {e}")
            ...
            while True:
                ...
                if collect_data and (now - last_data_time >= data_interval):
                    ...
                    try:
                        ...
                        print(f"Position: X={corrected_x_mm:.2f}, Y={corrected_y_mm:.2f}")
                    except Exception as e:
                        print(f"Data collection error: {e}")
    ...
    def simple_scan(self):
        ...
        # Send start marker to plotter (0,0) - top-right corner
        send_serial_data(0.0, 0.0)
        print("Sent start marker (0,0) to plotter - TOP-RIGHT corner")
        time.sleep(0.1)
        ...
        try:
            ...
            send_serial_data(final_x, final_y)
            print(f"Sent end marker ({final_x:.2f},{final_y:.2f}) to plotter")
        except Exception as e:
