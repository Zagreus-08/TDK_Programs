...
x, y, z = [], [], []
zmin, zmax = -0.1, 0.1
x_range = 100  # Default X-axis range (will grow to 680 max)
y_max = 100    # Default Y-axis maximum (will grow to 680 max)
...
def read_loop():
    ...
    while True:
        ...
        if x0 == 0 and y0 == 0:
            ...
            y_max = 50  # Start small, will grow to 680 max
            x_range = 50  # Start small, will grow to 680 max
        ...
        # Let X and Y grow but enforce 680x680 maximum square limit
        if scan_active and frozen_x_range is None:
            # Update X range independently but cap at 680
            if x0 > 0 and x0 <= 680:
                if x0 > x_range:
                    x_range = min(x0, 680)

            # Update Y range independently but cap at 680
            if y0 > 0 and y0 <= 680:
                if y0 > y_max:
                    y_max = min(y0, 680)
            
            # Force square: both axes must match the larger value, but max 680
            max_range = min(max(x_range, y_max), 680)
            x_range = y_max = max_range
            
            # At 90% through scan, freeze at 680x680
            if data_cnt == 1900:
                frozen_x_range = 680
                frozen_y_max = 680
                x_range = 680
                y_max = 680
                print(f"[INFO] *** AUTO-FREEZE at 90%: Display locked at 680x680mm ***")
        ...
        # ---------- End of scan ----------
        # Detect end of scan: hardware sends matching max values (680,680 for 680x680 scan)
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            ...
            # FREEZE the display range at 680x680 to prevent expansion
            frozen_x_range = 680
            frozen_y_max = 680
            x_range = 680
            y_max = 680
            print(f"[INFO] *** Display range FROZEN at: 680x680mm ***")
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    if len(xs) > 3000:
        step = max(1, len(xs) // 2500)  # Keep ~2500 points max for 680x680 data
        xs = xs[::step]
        ys = ys[::step]
        zs = zs[::step]
    ...
    if frozen_x_range is not None and frozen_y_max is not None:
        # Scan has completed - use frozen 680x680 values
        display_x_range = 680
        display_y_max = 680
    else:
        # Scan is active - use current ranges and keep them synced (max 680)
        max_range = min(max(x_range, y_max), 680)
        x_range = y_max = max_range
    ...
    # Limit grid resolution to max 140x140 for 680x680 live data (50% resolution for performance)
    ...
    # Use the display ranges (frozen or current) directly - always 680x680
    label_x_range = 680
    label_y_max = 680
...
def show_loaded(xs, ys, zs):
    ...
    # Auto-detect X and Y axis maximums from loaded data (cap at 680)
    detected_x_max = int(np.max(xs)) if len(xs) > 0 else 100
    detected_y_max = int(np.max(ys)) if len(ys) > 0 else 100

    # Cap at 680x680 maximum
    detected_x_max = min(detected_x_max, 680)
    detected_y_max = min(detected_y_max, 680)

    # Force square at 680x680
    x_range = 680
    y_max = 680

    print(f"[INFO] Detected X-max: {detected_x_max}, Y-max: {detected_y_max}")
    print(f"[INFO] Display set to: 680x680mm (square)")

    # Filter and scale data based on 680x680 range
    # Only show data where X <= 680 and Y <= 680
    mask = (xs <= 680) & (ys <= 680)
    xs = xs[mask]
    ys = ys[mask]
    zs = zs[mask]

    # Scale X and Y coordinates to fit in 0-100 display range (680x680 → 100x100)
    # Scanner sends: (0,0) at top-right, X increases left, Y increases down
    # Display wants: (0,0) at top-left, X increases right, Y increases down
    # Invert X axis (right to left) AND invert Y axis (bottom to top)
    xs = 100 - (xs * 100 / 680)  # Invert X: right becomes left
    ys = 100 - (ys * 100 / 680)  # Invert Y: bottom becomes top
    ...
    # Limit grid resolution to max 200x200 for loaded 680x680 data (higher quality for static viewing)
