...
- Main ains square 2D display with proper axis scaling
...
current_phase = 1  # 1 = horizontal (plot live), 2 = vertical (plot live), 3 = averaging complete data
phase1_data = {'x': [], 'y': [], 'z': []}  # Store Phase 1 complete data
phase2_data = {'x': [], 'y': [], 'z': []}  # Store Phase 2 complete data
...
def start_new_raw_file(name_hint=""):
    ...
    # Try multiple possible paths (Raspberry Pi and Windows)
    possible_raw_dirs = [
        '/home/pi/Shared/raw_data',  # Raspberry Pi
        r'c:\Users\a493353\Desktop\Lans Galos\Raspberry Pi Program\MIni XY Scanner\raw_data',  # Windows
...
def read_loop():
    ...
    global current_phase, phase1_data, phase2_data
    data_cnt = 0
    ...
    while True:
        ...
        if x0 == 0 and y0 == 0:
            ...
            # Clear Phase 1 and Phase 2 data storage
            phase1_data = {'x': [], 'y': [], 'z': []}
            phase2_data = {'x': [], 'y': [], 'z': []}
            current_phase = 1
            print("[INFO] Phase data cleared, starting Phase 1")
            ...
            y_max = 50  # Start small, will grow to 200 max
            x_range = 50  # Start small, will grow to 200 max
        ...
        # ---------- Create raw file on first data point if not already created ----------
        # This handles cases where scanner doesn't send (0,0) marker
        if raw_file is None and scan_active and data_cnt == 0:
            print("[INFO] First data received, creating raw file...")
            # Generate timestamp-based filename if no filename from serial
            from datetime import datetime
            if not filename_from_serial:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename_from_serial = f"scan_{timestamp}"
            start_new_raw_file(filename_from_serial)
            print(f"[INFO] Started raw file: {filename_from_serial}")
        ...
        # Let X and Y grow but enforce 200x200 maximum square limit
        if scan_active and frozen_x_range is None:
            # Update X range independently but cap at 200
            if x0 > 0 and x0 <= 200:
                if x0 > x_range:
                    x_range = min(x0, 200)

            # Update Y range independently but cap at 200
            if y0 > 0 and y0 <= 200:
                if y0 > y_max:
                    y_max = min(y0, 200)
            
            # Force square: both axes must match the larger value, but max 200
            max_range = min(max(x_range, y_max), 200)
            x_range = y_max = max_range
            
            # At 90% through scan, freeze at 200x200
            if data_cnt == 1900:
                frozen_x_range = 200
                frozen_y_max = 200
                x_range = 200
                y_max = 200
                print(f"[INFO] *** AUTO-FREEZE at 90%: Display locked at 200x200mm ***")

        data_cnt += 1

        # Detect phase transition from Phase 1 to Phase 2
        if phase == 2 and current_phase == 1:
            print(f"[INFO] Phase 1 complete! Saving horizontal scan data...")
            # Save Phase 1 data to separate CSV file
            try:
                if filename_from_serial:
                    phase1_filename = filename_from_serial + "_phase1_horizontal"
                else:
                    phase1_filename = "scan_phase1_horizontal"
                
                # Use same path logic as main raw data
                possible_paths = [
                    '/home/pi/Shared/raw_data',  # Raspberry Pi
                    r'c:\Users\a493353\Desktop\Lans Galos\Raspberry Pi Program\MIni XY Scanner\raw_data'  # Windows
                ]
                ...
                os.makedirs(raw_dir, exist_ok=True)
                phase1_path = os.path.join(raw_dir, f"{phase1_filename}.csv")
                
                with open(phase1_path, "w", newline="") as phase1_file:
                    phase1_writer = csv.writer(phase1_file)
                    phase1_writer.writerow(["x", "y", "z"])
                    for i in range(len(phase1_data['x'])):
                        phase1_writer.writerow([phase1_data['x'][i], phase1_data['y'][i], phase1_data['z'][i]])
                
                print(f"[INFO] Phase 1 data saved: {phase1_path} ({len(phase1_data['x'])} points)")
            except Exception as e:
                print(f"[ERROR] Failed to save Phase 1 data: {e}")

        # Handle data based on current phase
        if phase == 1:
            # Phase 1: Plot horizontal scan data live AND store for later averaging
            x.append(x0)
            y.append(y0)
            z.append(z0)
            # Also store in phase1_data for averaging later
            phase1_data['x'].append(x0)
            phase1_data['y'].append(y0)
            phase1_data['z'].append(z0)
            print(f"[Phase 1] Plotting: X={x0:.2f}, Y={y0:.2f}, Z={z0:.6f} (total: {len(x)})")
            
        elif phase == 2:
            # Phase 2: Plot vertical scan data live AND store for later averaging
            x.append(x0)
            y.append(y0)
            z.append(z0)
            # Also store in phase2_data for averaging later
            phase2_data['x'].append(x0)
            phase2_data['y'].append(y0)
            phase2_data['z'].append(z0)
            print(f"[Phase 2] Plotting: X={x0:.2f}, Y={y0:.2f}, Z={z0:.6f} (total: {len(x)})")
        else:
            # Unknown phase, treat as Phase 1
            x.append(x0)
            y.append(y0)
            z.append(z0)
            phase1_data['x'].append(x0)
        ...
        # ---------- End of scan ----------
        # Detect end of scan: hardware sends matching max values (200,200 for 200x200 scan)
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            print(f"[INFO] End of scan detected at ({x0},{y0}). Starting data averaging...")
            print(f"[DEBUG] Phase 1 data points: {len(phase1_data['x'])}")
            print(f"[DEBUG] Phase 2 data points: {len(phase2_data['x'])}")
            
            # Save Phase 2 data before averaging
            try:
                if filename_from_serial:
                    phase2_filename = filename_from_serial + "_phase2_vertical"
                else:
                    phase2_filename = "scan_phase2_vertical"
                ...
                os.makedirs(raw_dir, exist_ok=True)
                phase2_path = os.path.join(raw_dir, f"{phase2_filename}.csv")
                
                with open(phase2_path, "w", newline="") as phase2_file:
                    phase2_writer = csv.writer(phase2_file)
                    phase2_writer.writerow(["x", "y", "z"])
                    for i in range(len(phase2_data['x'])):
                        phase2_writer.writerow([phase2_data['x'][i], phase2_data['y'][i], phase2_data['z'][i]])
                
                print(f"[INFO] Phase 2 data saved: {phase2_path} ({len(phase2_data['x'])} points)")
            except Exception as e:
                print(f"[ERROR] Failed to save Phase 2 data: {e}")
            
            # FREEZE the display range at 200x200 to prevent expansion
            frozen_x_range = 200
            frozen_y_max = 200
            x_range = 200
            y_max = 200
            print(f"[INFO] *** Display range FROZEN at: 200x200mm ***")
            
            # ========== AVERAGE PHASE 1 AND PHASE 2 DATA ==========
            print("[INFO] Combining and averaging Phase 1 and Phase 2 data...")
            
            # Create dictionaries for fast lookup: {(x, y): z_value}
            phase1_dict = {}
            for i in range(len(phase1_data['x'])):
                coord_key = (round(phase1_data['x'][i], 1), round(phase1_data['y'][i], 1))
                if coord_key not in phase1_dict:
                    phase1_dict[coord_key] = []
                phase1_dict[coord_key].append(phase1_data['z'][i])
            
            phase2_dict = {}
            for i in range(len(phase2_data['x'])):
                coord_key = (round(phase2_data['x'][i], 1), round(phase2_data['y'][i], 1))
                if coord_key not in phase2_dict:
                    phase2_dict[coord_key] = []
                phase2_dict[coord_key].append(phase2_data['z'][i])
            
            # Average Phase 1 data at each coordinate
            for coord_key in phase1_dict:
                phase1_dict[coord_key] = sum(phase1_dict[coord_key]) / len(phase1_dict[coord_key])
            
            # Average Phase 2 data at each coordinate
            for coord_key in phase2_dict:
                phase2_dict[coord_key] = sum(phase2_dict[coord_key]) / len(phase2_dict[coord_key])
            
            # Combine all unique coordinates from both phases
            all_coords = set(phase1_dict.keys()) | set(phase2_dict.keys())
            ...
            for coord_key in all_coords:
                # Reconstruct actual coordinates from rounded key
                x_coord = coord_key[0]
                y_coord = coord_key[1]
                
                if coord_key in phase1_dict and coord_key in phase2_dict:
                    # Both phases have data - average them
                    z_avg = (phase1_dict[coord_key] + phase2_dict[coord_key]) / 2.0
                    averaged_count += 1
                elif coord_key in phase1_dict:
                    # Only Phase 1 has data
                    z_avg = phase1_dict[coord_key]
                    phase1_only += 1
                else:
                    # Only Phase 2 has data
                    z_avg = phase2_dict[coord_key]
                    phase2_only += 1
            ...
            print(f"[INFO] Averaging complete:")
            print(f"  - Points averaged from both phases: {averaged_count}")
            print(f"  - Points from Phase 1 only: {phase1_only}")
            print(f"  - Points from Phase 2 only: {phase2_only}")
            print(f"  - Total final points: {len(x)}")
            
            current_phase = 3  # Mark as averaging complete
            # ========== END AVERAGING ==========
            ...
            try:
                if filename_from_serial:
                    ...
                    possible_paths = [
                        ...
                        r'c:\Users\a493353\Desktop\Lans Galos\Raspberry Pi Program\MIni XY Scanner'  # Windows
                    ]
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    if frozen_x_range is not None and frozen_y_max is not None:
        # Scan has completed - use frozen 200x200 values
        display_x_range = 200
        display_y_max = 200
    else:
    ...
    display_name = ""
    if current_phase == 1:
        display_name = "Phase 1: Horizontal Scan"
    elif current_phase == 2:
        display_name = "Phase 2: Vertical Scan"
    elif current_phase == 3:
        display_name = "Averaged Result (Phase 1 + Phase 2)"
    else:
    ...
    # Use the display ranges (frozen or current) directly - always 200x200
    label_x_range = 200
    label_y_max = 200
...
def show_loaded(xs, ys, zs):
    ...
    # Cap at 200x200 maximum
    detected_x_max = min(detected_x_max, 200)
    detected_y_max = min(detected_y_max, 200)

    # Force square at 200x200
    x_range = 200
    y_max = 200

    print(f"[INFO] Detected X-max: {detected_x_max}, Y-max: {detected_y_max}")
    print(f"[INFO] Display set to: 200x200mm (square)")

    # Filter and scale data based on 200x200 range
    # Only show data where X <= 200 and Y <= 200
    mask = (xs <= 200) & (ys <= 200)
    xs = xs[mask]
    ys = ys[mask]
    zs = zs[mask]

    # Scale X and Y coordinates to fit in 0-100 display range (200x200 → 100x100)
    # Scanner sends: (0,0) at top-right, X increases left, Y increases down
    # Display wants: (0,0) at top-left, X increases right, Y increases down
    # Invert X axis (right to left) AND invert Y axis (bottom to top)
    xs = 100 - (xs * 100 / 200)  # Invert X: right becomes left
    ys = 100 - (ys * 100 / 200)  # Invert Y: bottom becomes top
...
if __name__ == '__main__':
    ...
    # Allow Escape to exit fullscreen if needed
