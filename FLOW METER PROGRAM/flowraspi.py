...
class MF5708Sensor:
    ...
    def _read_registers(self, start_reg, num_regs):
        if not self.connected or not self.serial:
            ...
            return None
        try:
            # Non-blocking-ish: clear buffer and write request, read in small bursts
            try:
...
class BaselineDialog:
    ...
    def _on_save(self):
        ...
        # prefer to save the current app total as the baseline for the chosen date/time
        try:
            baseline_value = float(self.parent._get_current_app_total())
        except Exception:
            baseline_value = 0.0
...
class FlowDashboard:
    def __init__(self, root):
        ...
        # keep previous total for reset detection (raw device based)
        self.prev_total = None

        # sync/auto-connect helpers
        self.total_offset = 0.0                       # software offset added to device total to produce app total
        self.device_total_before_disconnect = None   # device total stored when we disconnect
        self.app_total_before_disconnect = None      # app total at time of disconnect (device + offset)
        self.last_device_total = None                # last seen raw device total
        self.prev_device_total = None                # previous raw device total (for reset detection)
        self.auto_connect_enabled = True             # worker will auto-connect when port is present
        self.read_failures = 0                       # consecutive read failures counter
    ...
    def _build_ui(self):
        ...
        # clock (top-right of metrics bar)
        self.time_var = tk.StringVar(self.root, "")
        clock_font_size = 9 if IS_RASPBERRY_PI else 10
        self.time_label = ctk.CTkLabel(metrics_frame, textvariable=self.time_var,
                                       font=ctk.CTkFont(size=clock_font_size))
        self.time_label.place(relx=0.995, rely=0.06, anchor="ne")
        ...
        # smaller monthly figure for 800x480 displays
        self.fig_month, self.ax_month = plt.subplots(figsize=(5.0,4.0), dpi=90, constrained_layout=False)
        self.fig_month.patch.set_facecolor('#ffffff')
        self.ax_month.set_facecolor('#ffffff')
        self.ax_month.xaxis.set_major_formatter(mdates.DateFormatter("%d"))
        # Embed with slightly smaller padding to save vertical space
        self.canvas_month = FigureCanvasTkAgg(self.fig_month, master=month_frame)
        self.canvas_month.get_tk_widget().grid(row=0, column=0, sticky="nsew", padx=6, pady=6)
        ...
        # Left: Disconnect-only button (auto-enabled when we connect)
        left_controls = tk.Frame(bottom)
        left_controls.grid(row=0, column=0, sticky="w", padx=6, pady=bottom_pad)
        self.connect_btn = ctk.CTkButton(left_controls, text="Disconnect",
                                        width=small_btn_w, height=btn_h,
                                        command=self._disconnect_only)
        ...
        ctk.CTkButton(right_controls, text="Live", width=70, height=btn_h, command=self._goto_live_view).pack(side="left", padx=(0,6))
    ...
    # UI callbacks & remaining logic, adjusted for app_total (device + offset)
    def _refresh_ports(self):
        return get_available_ports()

    def _disconnect_only(self):
        """User requests to disconnect. We remember device/app totals and disable the button.
           Reconnection is handled automatically when the device/port reappears."""
        try:
            self.device_total_before_disconnect = getattr(self, "last_device_total", None)
            if self.device_total_before_disconnect is not None:
                self.app_total_before_disconnect = float(self.device_total_before_disconnect) + float(self.total_offset)
            else:
                self.app_total_before_disconnect = None
        except Exception:
            self.device_total_before_disconnect = None
            self.app_total_before_disconnect = None
        ...
        try:
            ...
            pass
        # keep auto-connect enabled so plugging the USB back will reconnect
    ...
    def _get_current_app_total(self):
        """Return the current app total (device reading + software offset)."""
        try:
            d = getattr(self, "last_device_total", None)
            if d is None and self.sensor and self.sensor.connected:
                res = self.sensor.read_all()
                if res:
                    d = res[1]
            if d is None:
                d = 0.0
            return float(d) + float(self.total_offset)
        except Exception:
            return float(self.total_offset)

    def _on_connected_sync(self):
        """Called once after we establish a connection. Reads the device and synchronizes
           the software offset if the meter reset while we were disconnected."""
        try:
            res = self.sensor.read_all()
            if not res:
                return
            flow, device_now, _ = res
            # update last device total trackers
            self.last_device_total = float(device_now)
            self.prev_device_total = float(device_now)

            prev_dev = self.device_total_before_disconnect
            prev_app = self.app_total_before_disconnect
            if prev_dev is not None and prev_app is not None:
                # If device decreased while disconnected -> meter reset/rollover
                if device_now < (prev_dev - 0.5):
                    # set software offset so app total remains continuous:
                    # total_offset = prev_app - device_now
                    self.total_offset = float(prev_app) - float(device_now)
                    try:
                        messagebox.showinfo("Sync", "Meter reset detected while disconnected. Totals synchronized.")
                    except Exception:
                        pass
                # Else: device increased while disconnected; leave total_offset unchanged (app_total continuity holds)
            # clear stored disconnect markers
            self.device_total_before_disconnect = None
            self.app_total_before_disconnect = None
        except Exception as e:
            if DEBUG_MODE:
                print("Connected sync error:", e, file=sys.stderr)

    def _toggle_connection(self):
        # This method retained for possible manual trigger; we do auto-connect in worker.
        if self.sensor.connected:
            self._disconnect_only()
        else:
            # Try to connect to detected port manually
            ...
            if self.sensor.connect():
                ...
                # sync on main thread
                try:
                    self.root.after(0, self._on_connected_sync)
                except Exception:
                    pass
            else:
    ...
    def _sensor_worker(self):
        ...
        while self.running:
            try:
                # auto-connect logic
                if not self.sensor.connected and self.auto_connect_enabled:
                    try:
                        ...
                        if port:
                            ...
                            self.sensor.baudrate = DEFAULT_SETTINGS["baud_rate"]
                            if self.sensor.connect():
                                # perform sync on GUI/main thread (messagebox must run on main thread)
                                try:
                                    self.root.after(0, self._on_connected_sync)
                ...
                start = time.time()
                # wrap read_all calls so exceptions are handled and counted
                try:
                    result = self.sensor.read_all()
                except Exception as e:
                    if DEBUG_MODE:
                        print("Worker read_all exception:", e, file=sys.stderr)
                    result = None

                if result is None:
                    # increment failure counter; if several consecutive failures then assume connection lost
                    self.read_failures += 1
                    if DEBUG_MODE:
                        print(f"[DEBUG] read failed (count={self.read_failures})", file=sys.stderr)
                    if self.read_failures >= 3:
                        # treat as disconnected
                        try:
                            if DEBUG_MODE:
                                print("[DEBUG] Treating device as disconnected after repeated read failures", file=sys.stderr)
                            self.sensor.disconnect()
                        except Exception:
                            pass
                        try:
                ...
                # protect thread from dying — log and sleep
                print("Sensor worker unexpected error:", e, file=sys.stderr)
                time.sleep(1.0)
    ...
    def _read_sensor(self):
        """Return (flow, device_total). When disconnected returns last_valid"""
        if not self.sensor.connected:
    ...
    def _do_update(self):
        ...
        flow, device_total = self._read_sensor()
        now = datetime.datetime.now()

        # remember raw device totals for reset detection and reconnection sync
        try:
            self.prev_device_total = getattr(self, "last_device_total", None)
            self.last_device_total = float(device_total)
        except Exception:
            pass

        # compute app total (device + software offset)
        try:
            app_total = float(device_total) + float(self.total_offset)
        except Exception:
            app_total = float(self.total_offset)

        # append app-level values to buffers
        self.times.append(now); self.flows.append(flow); self.totals.append(app_total)

        # daily baseline reset at midnight (or first reading after midnight)
        try:
            today = now.date()
            if self.daily_baseline_date != today:
                # set daily baseline using the app total so "Total Today" remains consistent
                self.daily_baseline = app_total
                self.daily_baseline_date = today
        except Exception:
            pass

        # detect meter reset / rollover based on raw device totals (not app_total)
        try:
            prev = self.prev_device_total
            if prev is not None and device_total < (prev - 0.5) and prev > 1.0:
                # meter likely reset while connected
                if DEBUG_MODE:
                    print(f"[DEBUG] Meter reset detected (connected): prev_dev={prev}, now_dev={device_total}", file=sys.stderr)
                try:
                    messagebox.showinfo("Meter reset", "Meter total decreased: possible meter reset. Baselines adjusted.")
                except Exception:
                    pass
                # set today's baseline to current app total
                self.daily_baseline = app_total
                ...
                if key not in self.baselines_by_month:
                    entry = {"value": float(app_total), "type": "auto", "ts": datetime.datetime.now().isoformat()}
                    self.baselines_by_month[key] = entry
        ...
        # auto baseline on month change: set baseline = current app_total when month advances
        try:
        ...
        # update displays and logging (use app_total)
        if self.mode == "live":
            try:
                self.var_flow.set(f"{flow:.2f} L/min")
                self.var_day.set(f"{max(0.0, app_total - self.daily_baseline):.3f} m³")
            except Exception:
                pass

            # month usage: use stored baseline if present, else try to infer; if none show raw app_total
            try:
                key = now.strftime("%Y-%m")
                entry = self._get_baseline_for_month(key)
                if entry:
                    baseline_val = float(entry.get("value", 0.0))
                    month_usage = max(0.0, app_total - baseline_val)
                    self.var_month.set(f"{month_usage:.3f} m³")
                else:
                    # no baseline yet — show raw app_total with note
                    self.var_month.set(f"{app_total:.3f} m³ (raw)")
            except Exception:
            ...
            try:
                # log using app_total (device + offset)
                append_log(flow, app_total)
            except Exception:
                pass
    ...
    def _update_clock(self):
        try:
            now = datetime.datetime.now()
            fmt = "%Y-%m-%d %H:%M:%S" if not IS_RASPBERRY_PI else "%H:%M:%S"
            self.time_var.set(now.strftime(fmt))
        except Exception:
            pass
    ...
    def _refresh_month_plot(self):
        ...
        try:
            ...
            for m in range(1,13):
                ...
                usages.append(usage)
                display_texts.append(f"{usage:.3f} m³" if usage > 0 else "-")
            ...
            self.ax_month.set_title(f"Monthly Usage ({year})")
            self.ax_month.grid(axis='x', alpha=0.2)

            max_usage = max(usages_rev) if any(usages_rev) else 0.0
            epsilon = 1e-12
            for i, (bar, txt) in enumerate(zip(bars, texts_rev)):
                w = bar.get_width()
                y = bar.get_y() + bar.get_height() / 2.0

                if w > 0 and max_usage > epsilon and w >= 0.10 * max_usage:
                    x = w * 0.5
                    ha = 'center'
                    color = 'white'
                elif w > 0:
                    x = w + (0.01 * (max_usage if max_usage > epsilon else 1.0))
                    ha = 'left'
                    color = 'black'
                else:
                    x = w + (0.01 * (max_usage if max_usage > epsilon else 1.0))
                ...
                self.ax_month.text(x, y, txt, va='center', ha=ha, color=color, fontsize=9, weight='bold', clip_on=False)
    ...
    def _goto_live_view(self):
        """Restore the real-time 'live' view (right-hand plot and metrics)."""
        try:
            self.mode = "live"
            ...
            try:
                ...
                self.var_flow.set(f"{flow:.2f} L/min")
                self.var_day.set(f"{max(0.0, app_total - self.daily_baseline):.3f} m³")
                entry = self._get_baseline_for_month(now.strftime("%Y-%m"))
                if entry:
                    baseline_val = float(entry.get("value", 0.0))
                    month_usage = max(0.0, app_total - baseline_val)
                    self.var_month.set(f"{month_usage:.3f} m³")
            ...
            print("Goto live view error:", e, file=sys.stderr)
