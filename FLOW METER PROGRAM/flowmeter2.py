# Compact MF5708 dashboard with compact settings + reboot/shutdown
import os
import sys
import csv
import time
...
# MF5708 sensor (unchanged parsing logic)
class MF5708Sensor:
...
# Logs Window (keeps minimal features)
class LogsWindow:
...
class FlowDashboard:
    def __init__(self, root):
        ...
        if IS_RASPBERRY_PI: self.root.attributes("-fullscreen", True)
        else: self.root.geometry("1300x840"); self.root.minsize(1100,650)
    ...
    def _build_ui(self):
        ...
        # left compact settings (narrow)
        left_w = 260 if IS_RASPBERRY_PI else 300
        panel = ctk.CTkFrame(self.root, width=left_w, corner_radius=8)
        panel.grid(row=0, column=0, sticky="nsew", padx=(8,6), pady=8)
        panel.grid_propagate(False)

        # Compact top row with small controls
        toprow = ctk.CTkFrame(panel, fg_color="transparent")
        toprow.pack(fill="x", padx=10, pady=(10,6))
        self.port_combo = ctk.CTkComboBox(toprow, values=get_available_ports(), width=140)
        self.port_combo.set(self.settings["com_port"] or (get_available_ports()[0] if get_available_ports() else "No port"))
        self.port_combo.pack(side="left", padx=(0,6))
        self.refresh_btn = ctk.CTkButton(toprow, text="⟳", width=36, height=28, command=self._refresh_ports)
        self.refresh_btn.pack(side="left")
        self.connect_btn = ctk.CTkButton(toprow, text="Connect", width=80, command=self._toggle_connection)
        self.connect_btn.pack(side="right")

        self.status_label = ctk.CTkLabel(panel, text="Disconnected", text_color="#b33434", font=ctk.CTkFont(size=10))
        self.status_label.pack(anchor="w", padx=10, pady=(6,6))

        # Baseline - compact
        bframe = ctk.CTkFrame(panel, fg_color="transparent")
        bframe.pack(fill="x", padx=10, pady=(4,8))
        ctk.CTkLabel(bframe, text="Monthly baseline:", font=ctk.CTkFont(size=11)).pack(anchor="w")
        self.baseline_label = ctk.CTkLabel(bframe, text=f"{self.monthly_baseline:.3f} m³", font=ctk.CTkFont(size=12, weight="bold"))
        self.baseline_label.pack(anchor="w", pady=(2,6))

        month_row = ctk.CTkFrame(bframe, fg_color="transparent")
        month_row.pack(fill="x")
        months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
        self.month_combo = ctk.CTkComboBox(month_row, values=months, width=120)
        self.month_combo.set(months[(datetime.datetime.now().month - 1) % 12])
        self.month_combo.pack(side="left")
        ctk.CTkButton(month_row, text="Set", width=60, command=self._set_baseline_for_selected_month).pack(side="right")

        ctk.CTkButton(panel, text="Set Baseline Now", width=left_w-20, command=self._set_monthly_baseline_now).pack(padx=10, pady=(8,6))
        ctk.CTkButton(panel, text="Clear Manual Baseline", width=left_w-20, fg_color="#f59e0b", command=self._clear_manual_baseline).pack(padx=10, pady=(0,8))

        # Advanced collapsible controls
        def toggle_adv():
            if self.adv_frame.winfo_ismapped():
                self.adv_frame.pack_forget()
                self.adv_toggle.configure(text="Show settings ▾")
            else:
                self.adv_frame.pack(fill="x", padx=10, pady=(6,8))
                self.adv_toggle.configure(text="Hide settings ▴")
        self.adv_toggle = ctk.CTkButton(panel, text="Show settings ▾", width=left_w-20, command=toggle_adv)
        self.adv_toggle.pack(padx=10, pady=(4,8))
        self.adv_frame = ctk.CTkFrame(panel, fg_color="#f7fafc", corner_radius=6)
        # initially hidden

        # update interval + window (small)
        ctk.CTkLabel(self.adv_frame, text="Update ms:", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=8, pady=(6,0))
        self.interval_slider = ctk.CTkSlider(self.adv_frame, from_=200, to=5000, command=self._on_interval_change, number_of_steps=48)
        self.interval_slider.set(self.settings["update_interval_ms"])
        self.interval_slider.pack(fill="x", padx=8, pady=(4,6))
        self.interval_label = ctk.CTkLabel(self.adv_frame, text=f"{self.settings['update_interval_ms']} ms", font=ctk.CTkFont(size=10))
        self.interval_label.pack(anchor="w", padx=8)

        ctk.CTkLabel(self.adv_frame, text="Graph sec:", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=8, pady=(6,0))
        self.window_slider = ctk.CTkSlider(self.adv_frame, from_=30, to=300, command=self._on_window_change, number_of_steps=27)
        self.window_slider.set(self.settings["graph_window_sec"])
        self.window_slider.pack(fill="x", padx=8, pady=(4,6))
        self.window_label = ctk.CTkLabel(self.adv_frame, text=f"{self.settings['graph_window_sec']} s", font=ctk.CTkFont(size=10))
        self.window_label.pack(anchor="w", padx=8, pady=(0,6))

        # Reboot / Shutdown (compact)
        ...
        ctk.CTkButton(bottom, text="Reboot", fg_color="#f59e0b", width=110, command=do_reboot).pack(side="left", padx=(0,8))
        ctk.CTkButton(bottom, text="Shutdown", fg_color="#dc3545", width=110, command=do_shutdown).pack(side="right")
        ...
        # Metrics row - smaller, neat
        metric_row = ctk.CTkFrame(right, fg_color="#f8fafc", corner_radius=6)
        metric_row.grid(row=0, column=0, sticky="ew", padx=8, pady=(8,6))
        ...
        mk_card(metric_row, self.var_flow, "Current").grid(row=0, column=0, padx=6, pady=8, sticky="nsew")
        mk_card(metric_row, self.var_day, "Total Today").grid(row=0, column=1, padx=6, pady=8, sticky="nsew")
        mk_card(metric_row, self.var_month, "Total This Month").grid(row=0, column=2, padx=6, pady=8, sticky="nsew")

        # Live plot
        self.fig_live, self.ax_live = plt.subplots(figsize=(9.5,4.5), dpi=100)
        ...
        self.canvas_live = FigureCanvasTkAgg(self.fig_live, master=right)
        self.canvas_live.get_tk_widget().grid(row=1, column=0, sticky="nsew", padx=8, pady=(4,8))

        # Controls row
        ctrl = ctk.CTkFrame(right, fg_color="#f8fafc", corner_radius=6)
        ctrl.grid(row=2, column=0, sticky="ew", padx=8, pady=(6,8))
        ctrl.grid_columnconfigure((0,1,2), weight=1)
        ctk.CTkButton(ctrl, text="Export CSV", width=120, command=self._export_csv).grid(row=0, column=0, padx=6, pady=8)
        ctk.CTkButton(ctrl, text="Data Logs", width=120, command=self._open_logs_window).grid(row=0, column=1, padx=6, pady=8)
        ctk.CTkButton(ctrl, text="Exit", fg_color="#dc3545", width=120, command=self._exit_now).grid(row=0, column=2, padx=6, pady=8)

        # Small monthly mini-plot in top-right corner of panel
        self.fig_month, self.ax_month = plt.subplots(figsize=(3.4,1.6), dpi=80)
        ...
        self.canvas_month = FigureCanvasTkAgg(self.fig_month, master=right)
        self.canvas_month.get_tk_widget().grid(row=0, column=0, sticky="ne", padx=(0,14), pady=(8,0))
    ...
    def _refresh_ports(self):
        ...
        if ports:
            ...
            self.settings["com_port"] = ports[0]

    def _on_interval_change(self, v):
        self.settings["update_interval_ms"] = int(float(v)); self.interval_label.configure(text=f"{self.settings['update_interval_ms']} ms")

    def _on_window_change(self, v):
        self.settings["graph_window_sec"] = int(float(v)); self.window_label.configure(text=f"{self.settings['graph_window_sec']} s")
