...
class DataScanClass:
    ...
    def GoCalib(self):
        ...
        # Make this window transient to the main window so it doesn't show
        # as a separate taskbar item. Also keep it on top and grab focus.
        try:
            popup.transient(self.gui.win)
        except Exception:
            pass
        try:
            popup.grab_set()
        ...
        try:
            ...
            # On Windows, toolwindow can prevent a taskbar entry for the Toplevel
        ...
        # Center popup relative to parent window (not the screen) to remain fully covered
        parent = self.gui.win
        try:
            ...
            # Fallback to default small screen center
            pw = 800
            ph = 480
            px = 0
            py = 0
        ...
        position_x = px + (pw // 2) - (popup_width // 2)
        position_y = py + (ph // 2) - (popup_height // 2)
...
class GUIClass(PortDefineClass):
    ...
    def __init__(self):
        ...
        self.logoFont = TkFont.Font(family='BiomeW04-Bold', size=50, weight='bold')
        ...
        self.label14 = Label(self.win, text = 'S/N: MAGS-PS01-022601', font = self.labelFont, height = 1, width = 21, bg='#0046ad', fg='white')
        ...
        self.scan_area_value = Label(self.win, text = str(StatusDataClass.roughness), font = self.labelFont3, width = 2, bg='#0046ad', fg='white')
        self.scan_area_value.place(x = 100, y = 335)
        ...
        self.clock_label = Label(self.win, text="", font=self.labelFont3, bg='#0046ad', fg='white')
        self.clock_label.place(x=5, y=5)
        self.clock_label.bind("<Button-1>", self.open_timezone_popup)
        
        

        # Load saved timezone (same as before)
        saved_tz = load_timezone_preference()
       # show current tz

        # Start clock update (same)
        self.update_clock()
        ...
        # Change RISING_EDGE to EITHER_EDGE
        self.cb = self.pi.callback(PortDefineClass.SWITCH, pigpio.EITHER_EDGE, self.emg_stop)
    ...
    def open_timezone_popup(self, event=None):
        """Open a centered popup for timezone selection"""
        popup = Toplevel(self.win)
        popup.title("Select Timezone")
        popup.geometry("380x420")
        popup.configure(bg="#e8f0fe")
        popup.transient(self.win)
        
        # --- FIX: Wait for the window to be drawn before grabbing ---
        popup.visibility_res = popup.wait_visibility() 
        ...
        popup.update_idletasks()
        x = self.win.winfo_rootx() + (self.win.winfo_width() // 2) - (380 // 2)
        y = self.win.winfo_rooty() + (self.win.winfo_height() // 2) - (420 // 2)
        ...
        # Scrollable frame
        frame = Frame(popup, bg="#e8f0fe")
        frame.pack(fill=BOTH, expand=True, padx=15, pady=5)
    
        canvas = Canvas(frame, bg="#e8f0fe", highlightthickness=0)
        scrollbar = Scrollbar(frame, orient="vertical", command=canvas.yview)
        scrollable_frame = Frame(canvas, bg="#e8f0fe")
        ...
        tz_var = StringVar(value=current_timezone.zone)
    
        # Define common timezones (Added this list as it was missing from your class)
        common_timezones = [
            'UTC', 'Asia/Tokyo', 'Asia/Seoul', 'Asia/Singapore', 'Asia/Manila', 
        ...
        # Buttons at bottom
        btn_frame = Frame(popup, bg="#e8f0fe")
        ...
        def apply_timezone():
            ...
            nonlocal tz_var
            selected = tz_var.get()
            if selected != current_timezone.zone:
                try:
                    current_timezone = pytz.timezone(selected)
                    save_timezone_preference(selected)
                    
                    print(f"[INFO] Timezone changed to: {selected}")
                    self.update_clock()
                except Exception as e:
                    print(f"Timezone change error: {e}")
                    messagebox.showerror("Error", f"Invalid timezone:\n{e}")
            popup.destroy()
        ...
        # Corrected binding (Inside the function, bound to the popup)
        popup.bind_all("<MouseWheel>", _on_mousewheel)
    # ---------------- Door Interlock ----------------
    def is_door_open(self):
        return (self.pi.read(self.LDoor) == 1 or 
                self.pi.read(self.RDoor) == 1)

    def monitor_doors(self):
        """Background monitor for doors. Stop motion if opened."""
        while self.running:
            door_currently_open = self.is_door_open()
            
            if door_currently_open:
                if not self.door_open:  # State change: Closed -> Open
                    self.door_open = True
                    SystemFuncClass.stop_flag = True
                    self.system_func.AllStop()
                    
                    # Store current task
                    current_text = self.label11.cget("text")
                    if current_text != "Door Open":
                        self.prev_axis_status = current_text
                    
                    # Update status to Door Open
                    self.label11.config(text="Door Open", bg="red", fg="white")
                    
                    # NOTE: label9 and label22 are NOT touched here, preserving their status.
                    
                    self._stop = False
                    self._flash_label()
                    self.win.after(0, self.update_button_states)

            else: # Doors are physically closed
                if self.door_open:  # State change: Open -> Closed
                    self.door_open = False
                    self._stop = True 
                    
                    # Restore state logic
                    active_tasks = ["Scanning", "Calibrating", "Homing", "Going to CalPos", "Going to ScanPos", "Unloading"]
                    if self.prev_axis_status in active_tasks:
                        self.label11.config(text="STOPPED", bg="red")
                    else:
                        self.label11.config(text=self.prev_axis_status, bg="#0046ad")
                    
                    # Reset motor flag and refresh the UI buttons
                    threading.Thread(target=self.reset_stop_flag_after_delay, daemon=True).start()
                    self.win.after(100, self.update_button_states)
    ...
    def emg_stop(self, gpio, level, tick):
        if level == 1: # Button is Pushed
            SystemFuncClass.stop_flag = True
            self.system_func.AllStop()
            self.label11.config(text="EMG STOP ACTIVE", bg="red", fg="white")
            self.label9.config(text="Not_Home", bg="red") # EMG breaks homing
            self.label22.config(text="Not Calibrated", bg="red") # EMG breaks calibration
            self.stop_timer()
            self.stopped_flashing()
            # Update UI to lock everything
            self.win.after(0, self.update_button_states)
        
        else: # level == 0 (Button is Released)
            self.label11.config(text="STOPPED", bg="red", fg="white")
            # Clear the stop flag after a safety delay
            threading.Thread(target=self.reset_stop_flag_after_delay, daemon=True).start()
            # Update UI to allow Home/Unload
            self.win.after(0, self.update_button_states)
    ...
    def update_clock(self):
        # Get current date and time in 12-hour format with AM/PM in selected timezone
        try:
    ...
    def scan_start(self):
        ...
        file_name1 = self.fname.get()
        scan_distance = self.label25.cget("text").strip()  # e.g. "2.0"
        fn = f'{file_name1}_{scan_distance}mm'
        ...
        # Get base filename from entry and sanitize it
        StatusDataClass.fn = self.get_filename_val().strip()
        if not StatusDataClass.fn:
            messagebox.showerror("Error", "No filename")
            self.scan.config(state='normal')
            return

        # Read scanning distance text (e.g. "2.0") from the label and append "mm"
        scan_distance = self.label25.cget("text").strip()  # e.g. "2.0"
        # Sanitize filename: remove dangerous characters and replace spaces with underscores
        safe_fn = "".join(ch for ch in StatusDataClass.fn if ch.isalnum() or ch in (' ', '-', '_')).rstrip()
        safe_fn = safe_fn.replace(' ', '_')

        # Construct final filename with scan distance appended (no timestamp)
        file_dir = '/home/pi/scanning_results'
        os.makedirs(file_dir, exist_ok=True)
        file_name = f'{file_dir}/{safe_fn}_{scan_distance}mm.csv'
        ...
        # Ensure ScanHt reflects current scanning-distance UI value
        try:
            StatusDataClass.ScanHt = int(float(self.label25.cget("text")) * 10) * 39
        except Exception as e:
            print(f"[WARN] Failed to parse scan distance: {e}")
    ...
    def init_offset_data(self):
        ...
        # Set initial button states based on axis status
        self.update_button_states()
    ...
    def goto_home(self):
        ...
        # Update button states based on new axis status
        self.update_button_states()
    ...
    def update_button_states(self):
        """Logic to enable/disable buttons based on Axis Status."""
        axis_status = self.label11.cget("text")
        
        # Check physical EMG state (Safety Override)
        emg_is_pushed = self.pi.read(PortDefineClass.SWITCH)

        # --- 1. ALWAYS ENABLED ---
        # Filename box is accessible at all times.
        self.fname.config(state='normal')

        # Define Button Groups for easier management
        system_btns = [self.ExitButton, self.RebootButton, self.ShutdownButton]
        
        # Specific Action Buttons requested for STOPPED state
        stopped_allowed_btns = [self.HomeButton, self.StopButton, self.UnloadButton]
        
        # All other primary action buttons
        other_action_btns = [self.scan, self.MoveScanButton, self.MoveCalibButton, 
                             self.CalibrateButton, self.ClearOffsetButton]
        
        # Adjustment Buttons
        xy_move_btns = [self.XPlusButton, self.XMinusButton, self.YPlusButton, self.YMinusButton]
        z_offset_btns = [self.inc_button, self.dec_button]
        scan_dist_btns = [self.up_button, self.down_button]
        scan_area_btns = [self.scan_area_minus, self.scan_area_plus]
        
        # Radio buttons 
        inputs = [self.res_100, self.res_150, self.res_200, self.low, self.med, 
                  self.high, self.xy_low, self.xy_med, self.single, self.demo]

        # Combine all UI for easy mass-disabling
        all_action_ui = stopped_allowed_btns + other_action_btns + xy_move_btns + \
                        z_offset_btns + scan_dist_btns + scan_area_btns + inputs

        # --- 2. EMERGENCY STOP (PHYSICAL) ---
        if emg_is_pushed == 1:
            for btn in all_action_ui + system_btns:
                btn.config(state='disabled')
            self.label11.config(text="EMG STOP ACTIVE", bg="red", fg="white")
            return

        # --- 3. DOOR OPEN ---
        if axis_status == "Door Open":
            for btn in all_action_ui + system_btns:
                btn.config(state='disabled')
            return

        # --- 4. ACTIVE MOVEMENT ---
        moving_states = ["Scanning", "Calibrating", "Homing", "Going to CalPos", "Going to ScanPos", "Unloading"]
        if axis_status in moving_states:
            for btn in all_action_ui:
                btn.config(state='disabled')
            # Enable specific system controls during movement
            self.StopButton.config(state='normal')
            for btn in system_btns:
                btn.config(state='normal')
            return

        # --- 5. STOPPED STATUS (After Stop or EMG Release) ---
        if axis_status == "STOPPED":
            # Disable everything first
            for btn in all_action_ui:
                btn.config(state='disabled')
            
            # Rule: Only Home, Stop, and Unload button will be enable
            for btn in stopped_allowed_btns:
                btn.config(state='normal')
                
            # Keep Exit/Reboot/Shutdown enabled
            for btn in system_btns:
                btn.config(state='normal')
            return

        # --- 6. STABLE POSITIONS (Home, Scan Pos, Calibration Pos) ---
        for btn in system_btns + stopped_allowed_btns: btn.config(state='normal')
        for btn in inputs: btn.config(state='normal')

        if axis_status == "Home":
            # Rule: Actions + Scan distance and Scan area buttons only
            for btn in other_action_btns + scan_dist_btns + scan_area_btns:
                btn.config(state='normal')
            # Disable Fine-tuning
            for btn in xy_move_btns + z_offset_btns:
                btn.config(state='disabled')

        elif axis_status == "Scan Pos":
            # Rule: Actions + Z-height and Move (XY) buttons only
            for btn in other_action_btns + xy_move_btns + z_offset_btns:
                btn.config(state='normal')
            # Disable Global Area/Distance
            for btn in scan_dist_btns + scan_area_btns:
                btn.config(state='disabled')

        elif axis_status == "Calibration Pos":
            # Rule: Actions + Move (XY) buttons only
            for btn in other_action_btns + xy_move_btns:
                btn.config(state='normal')
            # Disable Z-Offset and Global Area/Distance
            for btn in z_offset_btns + scan_dist_btns + scan_area_btns:
                btn.config(state='disabled')
