...
def read_loop():
    ...
    while True:
        ...
        try:
            ...
            # Parse z2 (data2) if present, otherwise default to 0
            z2 = float(parts[3]) if len(parts) >= 4 else 0.0
        ...
        if csv_writer and not (x0 == 0 and y0 == 0):
            try:
                ...
                csv_writer.writerow([x0, y0, z0, z2])
                # Flush every 100 rows to ensure data is saved (increased from 50 for better performance)
                if data_cnt % 100 == 0:
                    raw_file.flush()
                    print(f"[CSV] Saved {data_cnt} rows: X={x0:.2f}, Y={y0:.2f}, Z={z0:.6f}, Z2={z2:.6f}")
            except Exception as e:
