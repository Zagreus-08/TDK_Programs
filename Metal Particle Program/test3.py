...
Ver 1.8.0 Auto-Start Edition   2025-10-24
"""
...
def read_loop():
    ...
    while True:
        ...
        if int(x0) == 100 and int(y0) == 100 and not scan_finished:
            scan_finished = True
            update(None)  # plot once
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            save_path = os.path.join(os.path.expanduser("~"), f"scan_{timestamp}.png")
            save_queue.put((fig, save_path))
            print(f"🟢 Scan finished and plotted, saved to {save_path}")
...
def update(frame):
    """Continuous scanning plot update (auto-refresh)."""
...
# ------------------ Figure ------------------
fig = plt.figure('Scan System v1.8.0 Auto', figsize=[8, 3.8])
...
print("🚀 Auto-start scanning system running.")
root.mainloop()
