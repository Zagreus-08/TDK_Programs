...
if __name__ == '__main__':
    ...
    def detect_first_usb():
        """Detect the first available USB drive in /media/pi/"""
        try:
            media_path = '/media/pi'
            ...
            usb_devices = [d for d in os.listdir(media_path) if os.path.isdir(os.path.join(media_path, d))]
            
            if usb_devices:
                # Return first USB device path
                first_usb = os.path.join(media_path, usb_devices[0])
                print(f"[INFO] Detected first USB device: {first_usb}")
                return first_usb
            else:
                print("[WARNING] No USB devices found in /media/pi/")
                return None
        except Exception as e:
            print(f"[ERROR] Failed to detect USB device: {e}")
            return None
    ...
    def do_save():
        def custom_save():
            ...
            # Detect first available USB drive
            usb_path = detect_first_usb()
            
            if not usb_path:
                messagebox.showerror("No USB", "No USB drive detected!\nPlease insert a USB drive and try again.")
                return
            
            # Check if USB is writable
            if not os.access(usb_path, os.W_OK):
                messagebox.showerror("USB Error", f"USB drive is not writable:\n{usb_path}")
                return

            # Save PNG image to USB
            png_filename = os.path.join(usb_path, f"{base_filename}.png")
            
            try:
                # Save at exactly 800x480 pixels
                width_inches = 800 / 100
                height_inches = 480 / 100
                ...
                # Temporarily set exact size for saving
                fig.set_size_inches(width_inches, height_inches)
                fig.savefig(png_filename, dpi=100, bbox_inches=None)

                # Immediately restore original size
                fig.set_size_inches(original_size)
                canvas.draw()  # Refresh the display

                print(f"[INFO] PNG saved to USB: {png_filename}")
            except Exception as e:
                print(f"[ERROR] Failed to save PNG to USB: {e}")
            ...
            # Copy raw CSV file to USB
            csv_source = None
            if current_filename:
                # Use current live scan CSV
                csv_source = current_filename
            elif loaded_filename:
                # Find loaded CSV file in raw_data folder
                csv_source = os.path.join('/home/pi/Shared/raw_data', f"raw_{loaded_filename}.csv")
                if not os.path.exists(csv_source):
                    # Try without "raw_" prefix
                    csv_source = os.path.join('/home/pi/Shared/raw_data', f"{loaded_filename}.csv")
            
            if csv_source and os.path.exists(csv_source):
                try:
                    csv_destination = os.path.join(usb_path, f"{base_filename}.csv")
                    shutil.copy2(csv_source, csv_destination)
                    print(f"[INFO] CSV copied to USB: {csv_destination}")
                    messagebox.showinfo("Save Complete", f"Saved to USB:\n• {base_filename}.png\n• {base_filename}.csv")
                except Exception as e:
                    print(f"[ERROR] Failed to copy CSV to USB: {e}")
                    messagebox.showwarning("Partial Save", f"PNG saved successfully, but CSV copy failed:\n{e}")
            else:
                print(f"[WARNING] No CSV file found to copy (source: {csv_source})")
                messagebox.showinfo("Save Complete", f"PNG saved to USB:\n• {base_filename}.png\n\nNo CSV file available to copy.")
    ...
    def toggle_controls():
        if controls_frame.winfo_viewable():
            controls_frame.grid_remove()
            eject_btn.place_forget()  # Hide eject button when controls hidden
        else:
            controls_frame.grid()
            eject_btn.place(relx=0.99, rely=0.98, anchor="se", x=-160, y=-10)  # Show eject button when controls visible
    ...
    # Eject USB button overlay (bottom right corner of plot)
    def eject_usb():
        """Eject USB by closing file manager windows viewing the USB, then ejecting"""
        usb_path = detect_first_usb()
        
        if not usb_path:
            print("[WARNING] No USB drive detected for ejection")
            return
        
        device_name = os.path.basename(usb_path)
        
        if not os.path.exists(usb_path):
            print(f"[WARNING] USB path does not exist: {usb_path}")
            return
        
        try:
            # Step 1: Find the block device from the mount path
            result = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', usb_path], 
                                   capture_output=True, text=True, check=True)
            block_device = result.stdout.strip()
            print(f"[INFO] Found block device: {block_device} mounted at {usb_path}")
            
            # Step 2: Close any file handles we have open to the USB
            global raw_file
            if raw_file:
                try:
                    if hasattr(raw_file, 'name') and raw_file.name.startswith(usb_path):
                        raw_file.flush()
                        os.fsync(raw_file.fileno())
                        raw_file.close()
                        print(f"[INFO] Closed file handle to USB")
                        raw_file = None
                except Exception as e:
                    print(f"[WARNING] Error closing file handle: {e}")
            
            # Step 3: Close ALL pcmanfm file manager windows (not just ones viewing USB)
            # This is the KEY to preventing the "not properly ejected" warning
            print(f"[INFO] Closing all file manager windows...")
            try:
                subprocess.run(['-x', 'pcmanfm'], check=False, capture_output=True)
                time.sleep(0.5)  # Wait for windows to close
                print(f"[INFO] File manager windows closed")
            except Exception as e:
                print(f"[WARNING] Could not close file manager: {e}")
            
            # Step 4: Sync filesystem
            subprocess.run(['sync'], check=False, capture_output=True)
            time.sleep(0.5)
            print(f"[INFO] Synced filesystem")
            
            # Step 5: Use the eject command
            print(f"[INFO] Ejecting {block_device}...")
            subprocess.run(['sudo', 'eject', block_device], check=True, capture_output=True, text=True)
            
            print(f"[INFO] Successfully ejected: {device_name}")
            print(f"[INFO] USB is safe to remove!")
            
            # Show success notification
            messagebox.showinfo("USB Ejected", f"USB '{device_name}' successfully ejected!\n\nSafe to remove.")
            
        except subprocess.CalledProcessError as e:
            error_msg = e.stderr if e.stderr else str(e)
            print(f"[ERROR] Failed to eject {device_name}: {error_msg}")
            messagebox.showerror("Eject Failed", f"Failed to eject {device_name}:\n{error_msg}")
        except Exception as e:
            print(f"[ERROR] Error ejecting {device_name}: {e}")
            messagebox.showerror("Error", f"Error ejecting {device_name}:\n{e}")
    
    eject_btn = tk.Button(root, text="⏏ Eject USB", font=("Arial", 9, "bold"), bg="#ff5722", fg="white", 
                         relief="raised", command=eject_usb, width=10, height=1)
    # Don't place the button initially - it will be shown when controls panel is toggled on
    # eject_btn.place(relx=0.99, rely=0.98, anchor="se", x=-160, y=-10)
