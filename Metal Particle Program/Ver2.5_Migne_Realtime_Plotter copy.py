...
scan_area_x = 100  # Actual scan area X from metadata (will be updated from serial)
scan_area_y = 100  # Actual scan area Y from metadata (will be updated from serial)
display_range = 100  # Square display range (max of x_range and y_max, rounded down to nearest 10)
raw_file = None
csv_writer = None
...
def read_loop():
    ...
    global scan_area_x, scan_area_y, display_range
    data_cnt = 0
    ...
    while True:
        ...
        try:
            ...
            if parts[0] == "META" and len(parts) >= 3:
                try:
                    ...
                    # Calculate square display range (use max dimension, round down to nearest 10)
                    max_dim = max(scan_area_x, scan_area_y)
                    display_range = int(max_dim // 10) * 10  # Round down to nearest 10
                    
                    # Set both x_range and y_max to display_range for square display
                    x_range = display_range
                    y_max = display_range
                    
                    metadata_received = True
                    print(f"[INFO] Received scan area metadata: X={scan_area_x}mm, Y={scan_area_y}mm")
                    print(f"[INFO] Display range set to: {display_range}x{display_range}mm (square)")
                except Exception as e:
        ...
        if x0 == 0 and y0 == 0:
            ...
            metadata_received = False  # Reset metadata flag for new scan

            # Reset display range to default (will be updated by metadata)
            display_range = 100
            x_range = 100
            y_max = 100
        ...
        # NOTE: No longer auto-detect X/Y maximums from data - use metadata instead
        # The display_range is set from metadata and remains constant for square display
        ...
        if scan_active and data_cnt > 50 and metadata_received:
            # Check if we're at the end position (near scan_area_x and scan_area_y)
            at_end_x = abs(x0 - scan_area_x) <= 5  # Within 5mm tolerance
            at_end_y = abs(y0 - scan_area_y) <= 5  # Within 5mm tolerance
            
            if at_end_x and at_end_y:
...
def initialize_blank_plot():
    global x_range, y_max, display_range
    ax.cla()
    axh.cla()
    axm.cla()  # Clear the image subplot as well

    # Ensure display is always square using display_range
    x_range = display_range
    y_max = display_range

    ax.grid(True, linestyle="--", alpha=0.7)
    ax.set_xticks(np.arange(0, display_range + 1, max(10, int(display_range / 5))))
    ax.set_yticks(np.arange(0, 101, 20))
    ax.set_facecolor("white")

    axh.grid(True, linestyle="-", alpha=0.7)
    axh.set_xticks(np.arange(0, display_range + 1, max(10, int(display_range / 5))))
    ...
    # Adjust ticks to show display_range (BOTH AXES INVERTED)
    ax.set_xticks(np.linspace(0, 100, 6))
    # X-axis labels: max at left (0 display), 0 at right (100 display)
    ax.set_xticklabels([str(int(np.round((100-i) * display_range / 100))) for i in np.linspace(0, 100, 6)])
    axh.set_xticks(np.linspace(0, 100, 6))
    axh.set_xticklabels([str(int(np.round((100-i) * display_range / 100))) for i in np.linspace(0, 100, 6)])

    # Set Y-axis tick labels (INVERTED: max at top, 0 at bottom)
    ax.set_yticks(np.linspace(0, 100, 6))
    # Y-axis labels: max at top (0 display), 0 at bottom (100 display)
    ax.set_yticklabels([str(int(np.round((100-i) * display_range / 100))) for i in np.linspace(0, 100, 6)])
    axh.set_yticks(np.linspace(0, 100, 6))
    axh.set_yticklabels([str(int(np.round((100-i) * display_range / 100))) for i in np.linspace(0, 100, 6)])
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    global ax, axh, axm, cax, x_range, current_filename, y_max, zmin, zmax, display_range
    ...
    # Filter data based on display_range (square display)
    # Only show data where X <= display_range and Y <= display_range
    mask = [(xi <= display_range and yi <= display_range) for xi, yi in zip(xs, ys)]
    xs = [xs[i] for i in range(len(xs)) if mask[i]]
    ys = [ys[i] for i in range(len(ys)) if mask[i]]
    zs = [zs[i] for i in range(len(zs)) if mask[i]]

    # Ensure display is always square
    x_range = display_range
    y_max = display_range
    ...
    xs = [100 - (xi * 100 / display_range) for xi in xs]  # Invert X: right becomes left
    ys = [100 - (yi * 100 / display_range) for yi in ys]  # Invert Y: bottom becomes top
...
def show_loaded(xs, ys, zs):
    global zmin, zmax, ax, axh, axm, cax, x_range, y_max, loaded_data_cache, display_range
    ...
    # Calculate square display range (use max dimension, round down to nearest 10)
    max_dim = max(detected_x_max, detected_y_max)
    display_range = int(max_dim // 10) * 10  # Round down to nearest 10
    ...
    print(f"[INFO] Loaded data - Detected X-max: {detected_x_max}, Y-max: {detected_y_max}")
    print(f"[INFO] Display range set to: {display_range}x{display_range}mm (square)")

    # Filter data based on display_range (square display)
    mask = (xs <= display_range) & (ys <= display_range)
    xs = xs[mask]
    ys = ys[mask]
    zs = zs[mask]
    ...
    # Display wants: (0,0) at top-left, X increases right, Y increases down
    # Invert X axis (right to left) AND invert Y axis (bottom to top)
    xs = 100 - (xs * 100 / display_range)  # Invert X: right becomes left
    ys = 100 - (ys * 100 / display_range)  # Invert Y: bottom becomes top
...
def resume_live():
    ...
    global pause_live, x, y, z, zmin, zmax, ax, axh, axm, cax, loaded_filename, loaded_data_cache, display_range
    pause_live = False
    ...
    # Reset display_range to default
    display_range = 100
