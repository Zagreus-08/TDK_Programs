...
current_phase = None  # 'horizontal' or 'vertical'
phase1_complete = False  # Track if Phase 1 (horizontal) is done
last_y_position = None  # Track Y position to detect phase changes
...
# ---------------- Raw Data Handling ----------------
def start_new_raw_file(name_hint="", phase_suffix=""):
    """Start a new CSV for saving live scan data with optional phase suffix"""
    global raw_file, csv_writer, current_filename
    if not name_hint:
        # Use simple sequential naming if no hint provided
        name_hint = "scan_data"
    
    # Add phase suffix if provided (e.g., "_Phase1_Horizontal" or "_Phase2_Vertical")
    if phase_suffix:
        name_hint = f"{name_hint}{phase_suffix}"
...
def detect_scan_phase(x0, y0):
    """
    Detect if current scan is horizontal (Phase 1) or vertical (Phase 2)
    
    Phase 1 (Horizontal): X changes while Y stays relatively constant
    Phase 2 (Vertical): Y changes while X stays relatively constant
    
    Returns: 'horizontal', 'vertical', or None if cannot determine yet
    """
    global last_y_position, current_phase
    
    # Need at least a few data points to determine phase
    if len(x) < 5:
        last_y_position = y0
        return None
    
    # Check if Y position is changing significantly (vertical scan)
    if last_y_position is not None:
        y_change = abs(y0 - last_y_position)
        
        # If Y is changing more than 5mm between readings, it's likely vertical
        if y_change > 5:
            last_y_position = y0
            return 'vertical'
        
        # If Y stays relatively constant (< 2mm change), it's horizontal
        if y_change < 2:
            last_y_position = y0
            return 'horizontal'
    
    last_y_position = y0
    return current_phase  # Return current phase if we can't determine
...
def read_loop():
    ...
    global current_phase, phase1_complete, last_y_position
    data_cnt = 0
    filename_from_serial = ""
    base_filename = ""  # Store base filename without phase suffix
    ...
    while True:
        ...
        if x0 == 0 and y0 == 0:
            ...
            # Reset phase detection variables
            current_phase = None
            phase1_complete = False
            last_y_position = None
            ...
            if len(parts) >= 4:
                ...
                # Don't create file yet - wait to detect phase first
        ...
        # ---------- Phase Detection ----------
        if scan_active and data_cnt > 0:
            detected_phase = detect_scan_phase(x0, y0)
            ...
            if detected_phase and detected_phase != current_phase:
                ...
                # Close previous phase file if it exists
                if raw_file and old_phase:
                    try:
                        raw_file.flush()
                        raw_file.close()
                        print(f"[INFO] Closed Phase file: {current_filename}")
                    except Exception:
                        pass
                    raw_file = None
                    csv_writer = None
                ...
                if current_phase == 'horizontal':
                    if not phase1_complete:
                        phase_suffix = "_Phase1_Horizontal"
                        print("[INFO] *** PHASE 1 DETECTED: Horizontal Scan ***")
                    else:
                        phase_suffix = "_Phase1b_Horizontal"
                        print("[INFO] *** Additional Horizontal Scan Detected ***")
                elif current_phase == 'vertical':
                    phase_suffix = "_Phase2_Vertical"
                    phase1_complete = True  # Mark Phase 1 as complete
                    print("[INFO] *** PHASE 2 DETECTED: Vertical Scan ***")
                else:
                    phase_suffix = ""
                
                # Create new file for this phase
                if base_filename:
                    start_new_raw_file(base_filename, phase_suffix)
                else:
                    start_new_raw_file("scan_data", phase_suffix)
        ...
        # For 190x190mm scan, look for (190,190) or close to it
        # Only trigger if we have enough data points (avoid false positives)
        if scan_active and data_cnt > 50:
            # Check if we're at the end of scan (around 190,190 for actual scan area)
            if (abs(x0 - scan_area_x) <= 5 and abs(y0 - scan_area_y) <= 5) or (x0 >= 185 and y0 >= 185 and abs(x0 - y0) <= 5):
                print(f"[INFO] End of scan detected at ({x0},{y0}). Finalizing scan...")
                print(f"[DEBUG] Scan area: {scan_area_x}x{scan_area_y}mm")
                print(f"[DEBUG] Max data point: X={max(x) if x else 0}, Y={max(y) if y else 0}")
                
                # FIXED: Display range stays at 920x920mm
                frozen_x_range = FIXED_DISPLAY_X
                frozen_y_max = FIXED_DISPLAY_Y
                print(f"[INFO] *** Display range FIXED at: {frozen_x_range}x{frozen_y_max}mm ***")
            ...
            try:
                if filename_from_serial:
                    ...
                    print(f"[INFO] Scan complete, attempting to save PNG to: {save_path}")
