...
if __name__ == '__main__':
    ...
    def eject_usb():
        """Eject the first available USB drive - NO confirmation or success dialogs"""
        usb_path = detect_first_usb()
        ...
        # Check if path exists and is actually mounted before attempting unmount
        if not os.path.exists(usb_path):
            print(f"[WARNING] USB path does not exist: {usb_path}")
            return
        
        try:
            # Sync filesystem first to ensure all writes are complete
            subprocess.run(['sync'], check=False, capture_output=True)
            
            # Use sudo umount - this is the CRITICAL method that works properly
            result = subprocess.run(['sudo', 'umount', usb_path], check=True, capture_output=True)
            print(f"[INFO] Successfully ejected: {device_name} from {usb_path}")
            
            # Verify it's actually unmounted by checking if path is still a mount point
            check_result = subprocess.run(['mountpoint', '-q', usb_path], capture_output=True)
            if check_result.returncode != 0:
                print(f"[INFO] Verified: {usb_path} is no longer mounted")
            else:
                print(f"[WARNING] {usb_path} still appears to be mounted")
                
            # No messagebox - silent eject, only console logging
