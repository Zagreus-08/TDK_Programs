...
loaded_fullpath = None   # Track full path of loaded raw CSV (for copying to USB)
...
possible_image_paths = [
    '/home/pi/Migne_black_frameless.png',  # Raspberry Pi /home/pi
    '/home/pi/Desktop/Migne_black_frameless.png',  # Raspberry Pi Desktop
...
# ---------------- Load Raw CSV ----------------
def load_raw_data():
    global pause_live, loaded_filename, loaded_fullpath
    pause_live = True
    ...
    tk.Button(btn_frame, text="Save to USB", command=do_save_to_usb, font=("Arial", 11, "bold"),
    ...
    try:
        ...
        # Store full path so we can copy original file to USB if requested
        loaded_fullpath = file_path
...
if __name__ == '__main__':
    ...
    def do_save_to_usb():
        """Open dialog to select raw CSV and image, then choose USB folder to copy files to."""
        def open_dialog():
            global loaded_fullpath, loaded_data_cache

            # Possible source directories to search for CSV/PNG
            source_dirs = ['/home/pi/Shared/raw_data', '/home/pi/Shared', os.getcwd()]
            src_dir = next((d for d in source_dirs if os.path.exists(d)), os.getcwd())
            ...
            # Detect if this is a small screen (7-inch Raspberry Pi display)
            is_small_screen = screen_w <= 1024 or screen_h <= 600
            
            if is_small_screen:
                # For small screens, use full screen width minus margins
                w = screen_w - 20
                h = screen_h - 40
            else:
                # For larger screens, use fixed size
                w = min(700, max(480, screen_w - 20))
                h = min(550, max(320, screen_h - 40))
            ...
            # Use consistent fonts with Load Raw dialog - smaller on small screens
            if is_small_screen:
            ...
            main_container = tk.Frame(dialog, bg="#e5e5e5")
            main_container.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)

            if is_small_screen:
                # SAME LAYOUT AS PC - Vertical stacking on left, USB on right
                left_col = tk.Frame(main_container, bg="#e5e5e5")
                left_col.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=4)

                csv_frame = tk.Frame(left_col, bg="#e5e5e5")
                csv_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=4)

                png_frame = tk.Frame(left_col, bg="#e5e5e5")
                png_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=4)

                # Right side for USB and buttons
                right_w = 180  # Fixed width for right column on small screens
                right_frame = tk.Frame(main_container, width=right_w, bg="#e5e5e5")
                right_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=4)
                right_frame.pack_propagate(False)
                
                button_container = right_frame  # Buttons go in right frame
                
            else:
                # HORIZONTAL LAYOUT for large screens (original layout)
                left_col = tk.Frame(main_container, bg="#e5e5e5")
                left_col.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=4)
                ...
                right_w = max(140, int(w * 0.18))
                right_frame = tk.Frame(main_container, width=right_w, bg="#e5e5e5")
                right_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=4)
                right_frame.pack_propagate(False)
                
                button_container = right_frame  # Buttons go in right frame for large screens

            # CSV list as scrollable checkboxes
            tk.Label(csv_frame, text="CSV Files", font=lbl_font, bg="#e5e5e5").pack(anchor="w")
            csv_canvas = tk.Canvas(csv_frame, bg="white")
            csv_scroll = tk.Scrollbar(csv_frame, command=csv_canvas.yview, width=8)
            csv_scroll.pack(side=tk.RIGHT, fill=tk.Y)
            csv_canvas.pack(fill=tk.BOTH, expand=True)
            csv_checks_frame = tk.Frame(csv_canvas, bg="white")
            csv_canvas.create_window((0, 0), window=csv_checks_frame, anchor='nw')
            csv_canvas.config(yscrollcommand=csv_scroll.set)

            # PNG list as scrollable checkboxes
            tk.Label(png_frame, text="PNG Files", font=lbl_font, bg="#e5e5e5").pack(anchor="w")
            png_canvas = tk.Canvas(png_frame, bg="white")
            png_scroll = tk.Scrollbar(png_frame, command=png_canvas.yview, width=8)
            png_scroll.pack(side=tk.RIGHT, fill=tk.Y)
            png_canvas.pack(fill=tk.BOTH, expand=True)
            png_checks_frame = tk.Frame(png_canvas, bg="white")
            png_canvas.create_window((0, 0), window=png_checks_frame, anchor='nw')
            png_canvas.config(yscrollcommand=png_scroll.set)

            # Dictionaries to hold checkbutton variables
            csv_vars = {}
            png_vars = {}

            def _on_canvas_config(event, canvas=csv_canvas):
                canvas.configure(scrollregion=canvas.bbox('all'))
            def _on_canvas_config_png(event, canvas=png_canvas):
                canvas.configure(scrollregion=canvas.bbox('all'))
            csv_checks_frame.bind('<Configure>', _on_canvas_config)
            png_checks_frame.bind('<Configure>', _on_canvas_config_png)

            # Select/Clear buttons
            csv_btns = tk.Frame(csv_frame, bg="#e5e5e5")
            csv_btns.pack(fill=tk.X)
            def csv_select_all():
                for var, _ in csv_vars.values():
                    var.set(True)
            def csv_clear_all():
                for var, _ in csv_vars.values():
                    var.set(False)
            btn_width = 8 if is_small_screen else 10
            tk.Button(csv_btns, text="Select All", command=csv_select_all, width=btn_width, font=btn_font).pack(side=tk.LEFT, padx=2, pady=2)
            tk.Button(csv_btns, text="Clear All", command=csv_clear_all, width=btn_width, font=btn_font).pack(side=tk.LEFT, padx=2, pady=2)

            png_btns = tk.Frame(png_frame, bg="#e5e5e5")
            png_btns.pack(fill=tk.X)
            def png_select_all():
                for var, _ in png_vars.values():
                    var.set(True)
            def png_clear_all():
                for var, _ in png_vars.values():
                    var.set(False)
            tk.Button(png_btns, text="Select All", command=png_select_all, width=btn_width, font=btn_font).pack(side=tk.LEFT, padx=2, pady=2)
            tk.Button(png_btns, text="Clear All", command=png_clear_all, width=btn_width, font=btn_font).pack(side=tk.LEFT, padx=2, pady=2)

            # USB mounts list
            tk.Label(right_frame, text="USB Targets", font=lbl_font, bg="#e5e5e5").pack(anchor="w", padx=2)
            
            # USB listbox: fixed small height since Pi only has 2 USB ports
            usb_listbox = tk.Listbox(right_frame, selectmode=tk.SINGLE, font=("Courier", 9 if is_small_screen else 10), height=3)
            usb_listbox.pack(fill=tk.X, expand=False, padx=2, pady=2)
            usb_scroll = tk.Scrollbar(right_frame, command=usb_listbox.yview, width=15)
            usb_scroll.pack(side=tk.RIGHT, fill=tk.Y)
            usb_listbox.config(yscrollcommand=usb_scroll.set)
            usb_listbox.config(yscrollcommand=usb_scroll.set)

            # Populate file lists
            def populate_files(directory):
                # Clear existing
                for w in csv_checks_frame.winfo_children():
                    w.destroy()
                for w in png_checks_frame.winfo_children():
                    w.destroy()
                csv_vars.clear()
                png_vars.clear()
                
                # Search in multiple directories for PNG files
                png_search_paths = [
                    directory,  # Current directory
                    '/home/pi/Shared',  # Main Shared folder (where PNG files are auto-saved)
                    '/home/pi/Shared/raw_data',  # Raw data subfolder
                ]
                ...
                try:
                    ...
                    for f in sorted(files, reverse=True):
                        full_path = os.path.join(directory, f)
                        if f.lower().endswith('.csv') and os.path.isfile(full_path):
                            all_csv_files.append((f, full_path))
                except Exception as e:
                    print(f"[ERROR] Failed to list CSV files: {e}")
                
                # Search for PNG files in multiple locations
                for search_path in png_search_paths:
                    ...
                    try:
                        ...
                        for f in sorted(files, reverse=True):
                            ...
                            if f.lower().endswith('.png') and os.path.isfile(full_path):
                                # Avoid duplicates by checking if filename already exists
                                if not any(png[0] == f for png in all_png_files):
                                    all_png_files.append((f, full_path))
                    except Exception as e:
                        print(f"[ERROR] Failed to list PNG files in {search_path}: {e}")
                
                # Create checkboxes for CSV files
                for filename, full_path in all_csv_files:
                    var = tk.BooleanVar(value=False)
                    cb = tk.Checkbutton(csv_checks_frame, text=filename, variable=var, anchor='w', font=cb_font, bg="white")
                    cb.pack(anchor='w', fill=tk.X)
                    csv_vars[filename] = (var, full_path)  # Store both var and full path
                
                # Create checkboxes for PNG files
                for filename, full_path in all_png_files:
                    var = tk.BooleanVar(value=False)
                    cb = tk.Checkbutton(png_checks_frame, text=filename, variable=var, anchor='w', font=cb_font, bg="white")
                    cb.pack(anchor='w', fill=tk.X)
                    png_vars[filename] = (var, full_path)  # Store both var and full path
                
                print(f"[INFO] Found {len(all_csv_files)} CSV files and {len(all_png_files)} PNG files")
                ...
                try:
                    ...
                    for d in os.listdir(base):
                        ...
                        if os.path.isdir(full_path):
                            # Get volume label using lsblk command
                            try:
                                # Find the device that's mounted at this path
                                result = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', full_path], 
                                                      capture_output=True, text=True, timeout=2)
                                if result.returncode == 0:
                                    device = result.stdout.strip()
                                    # Get label for this device
                                    label_result = subprocess.run(['lsblk', '-n', '-o', 'LABEL', device], 
                                                                 capture_output=True, text=True, timeout=2)
                                    label = label_result.stdout.strip()
                                    # If we got a label, use it; otherwise use folder name
                                    display_name = label if label else d
                                else:
                                    display_name = d
                            except Exception:
                                # If command fails, just use folder name
                                display_name = d
                    ...
                    for display_name, full_path in entries:
                        usb_listbox.insert(tk.END, display_name)
                        # Store the full path as a mapping (we'll need it for eject)
                        if not hasattr(usb_listbox, 'path_mapping'):
                            usb_listbox.path_mapping = {}
                        usb_listbox.path_mapping[display_name] = full_path
                    
                except Exception as e:
                    print(f"[ERROR] Failed to populate USB list: {e}")

            # Directory label and change button - adjust for small screens
            dir_var = tk.StringVar(value=src_dir)
            if not is_small_screen:
                dir_frame = tk.Frame(right_frame, bg="#e5e5e5")
                dir_frame.pack(fill=tk.X, padx=2, pady=4)
                tk.Label(dir_frame, textvariable=dir_var, font=cb_font, bg="#e5e5e5").pack(side=tk.LEFT, padx=2)
                def change_dir():
                    d = filedialog.askdirectory(initialdir=dir_var.get())
                    if d:
                        dir_var.set(d)
                        populate_files(d)
                tk.Button(dir_frame, text="Change Folder", command=change_dir, font=("Arial", 8, "bold")).pack(side=tk.RIGHT)

            populate_files(src_dir)
            populate_usb()

            # Preselect loaded file if present
            if loaded_fullpath:
                lf = os.path.basename(loaded_fullpath)
                try:
                    if lf in csv_vars:
                        csv_vars[lf].set(True)
                except Exception:
                    pass

            # Action buttons - place inside button_container
            def do_copy():
                sel_usb = None
                s3 = usb_listbox.curselection()
                if s3:
                    sel_display = usb_listbox.get(s3[0])
                    # Get the actual path from the mapping
                    if hasattr(usb_listbox, 'path_mapping') and sel_display in usb_listbox.path_mapping:
                        sel_usb = usb_listbox.path_mapping[sel_display]
                    else:
                        # Fallback: assume it's under /media/pi
                        sel_usb = os.path.join('/media/pi', sel_display)

                if not sel_usb or not os.path.exists(sel_usb):
                    messagebox.showerror("No USB", "Please select a valid USB target folder.")
                    return

                copied = []
                try:
                    # CSVs selected via checkboxes
                    for fname, (var, full_path) in csv_vars.items():
                        if var.get():
                            if os.path.exists(full_path):
                                dst = os.path.join(sel_usb, os.path.basename(full_path))
                                shutil.copy2(full_path, dst)
                                copied.append(dst)

                    # PNGs selected via checkboxes
                    for fname, (var, full_path) in png_vars.items():
                        if var.get():
                            if os.path.exists(full_path):
                                dst = os.path.join(sel_usb, os.path.basename(full_path))
                                shutil.copy2(full_path, dst)
                ...
                if copied:
                    messagebox.showinfo("Done", f"Copied {len(copied)} file(s) successfully!")
                else:
                    messagebox.showwarning("Nothing", "No files selected to copy.")
                dialog.destroy()

            def do_cancel():
                dialog.destroy()

            def do_eject_selected():
                # Eject/unmount the selected USB target from the listbox - NO CONFIRMATION, NO SUCCESS MESSAGE
                s3 = usb_listbox.curselection()
                if not s3:
                    messagebox.showwarning("No Selection", "Please select a USB target to eject.")
                    return
                ...
                # Get the actual path from the mapping
                if hasattr(usb_listbox, 'path_mapping') and sel_display in usb_listbox.path_mapping:
                    target = usb_listbox.path_mapping[sel_display]
                else:
                    # Fallback: assume it's under /media/pi
                    target = os.path.join('/media/pi', sel_display)
                
                if not os.path.exists(target):
                    messagebox.showerror("Not Found", f"Target does not exist:\n{target}")
                    return
                
                # Direct eject without confirmation or success message
                try:
                    # First try a plain unmount (no sudo) which works when program runs as root
                    res = subprocess.run(['umount', target], capture_output=True, text=True)
                    if res.returncode == 0:
                        print(f"[INFO] Successfully unmounted: {sel_display}")
                        populate_usb()
                    else:
                        # Try to resolve device and use udisksctl if available
                        device = None
                        try:
                            fm = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', target], capture_output=True, text=True, timeout=2)
                            if fm.returncode == 0:
                                device = fm.stdout.strip()
                        except Exception:
                            device = None

                        # Try udisksctl unmount if present
                        udisks = shutil.which('udisksctl')
                        if udisks and device:
                            try:
                                r2 = subprocess.run([udisks, 'unmount', '-b', device], capture_output=True, text=True, check=True)
                                print(f"[INFO] Successfully unmounted via udisksctl: {sel_display} ({device})")
                                populate_usb()
                                continue
                            except subprocess.CalledProcessError as e:
                                pass

                        # As a last resort try sudo umount (may prompt for password and fail)
                        try:
                            r3 = subprocess.run(['sudo', 'umount', target], capture_output=True, text=True, check=True)
                            print(f"[INFO] Successfully unmounted with sudo: {sel_display}")
                            populate_usb()
                        except subprocess.CalledProcessError as e:
                            err = e.stderr or e.stdout or str(e)
                            messagebox.showerror("Eject Failed", f"Failed to eject {sel_display}:\n{err}")
                except Exception as e:
                    messagebox.showerror("Error", f"Error ejecting {sel_display}:\n{e}")
            ...
            tk.Button(button_container, text="Transfer to USB", command=do_copy, bg="#4CAF50", fg="white", 
                     font=btn_font, width=btn_w, height=btn_h).pack(pady=btn_pady, padx=2)
            tk.Button(button_container, text="Eject USB", command=do_eject_selected, bg="#ff9800", fg="white", 
                     font=btn_font, width=btn_w, height=btn_h).pack(pady=btn_pady, padx=2)
            tk.Button(button_container, text="Cancel", command=do_cancel, bg="#f44336", fg="white", 
                     font=btn_font, width=btn_w, height=btn_h).pack(pady=btn_pady, padx=2)

            dialog.wait_window()

        safe_action(open_dialog)

    def do_eject_usb():
        """Unmount a selected USB mount under /media/pi."""
        def open_eject_dialog():
            base = '/media/pi'
            if not os.path.exists(base):
                messagebox.showerror("No USB", f"No USB mount point found: {base}")
                return

            dialog = tk.Toplevel(root)
            dialog.title("Eject USB")
            dialog.geometry("400x300")
            dialog.transient(root)
            dialog.grab_set()

            tk.Label(dialog, text="Select USB target to eject:", font=("Arial", 11, "bold")).pack(anchor="w", padx=8, pady=6)
            listbox = tk.Listbox(dialog, selectmode=tk.SINGLE, font=("Courier", 10))
            listbox.pack(fill=tk.BOTH, expand=True, padx=8, pady=4)

            # Populate with volume labels and store path mapping
            listbox.path_mapping = {}
            try:
                for d in sorted(os.listdir(base)):
                    ...
                    if os.path.isdir(full_path):
                        ...
                        listbox.path_mapping[display_name] = full_path
            except Exception:
                pass
            ...
            def do_unmount():
                ...
                if not sel:
                    messagebox.showwarning("No Selection", "Please select a target to eject.")
                    return
                
                sel_display = listbox.get(sel[0])
                target = listbox.path_mapping.get(sel_display, os.path.join(base, sel_display))
                
                if not os.path.exists(target):
                    messagebox.showerror("Not Found", f"Target does not exist:\n{target}")
                    return
                
                # Try to unmount cleanly with multiple fallbacks and show result
                try:
                    # Try umount first
                    res = subprocess.run(['umount', target], capture_output=True, text=True)
                    if res.returncode == 0:
                        messagebox.showinfo("Ejected", f"Successfully ejected: {sel_display}")
                        dialog.destroy()
                        return

                    # Resolve device for alternative methods
                    device = None
                    try:
                        fm = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', target], capture_output=True, text=True, timeout=2)
                        if fm.returncode == 0:
                    ...
                    # Try udisksctl if available
                    udisks = shutil.which('udisksctl')
                    if udisks and device:
                        try:
                            subprocess.run([udisks, 'unmount', '-b', device], capture_output=True, text=True, check=True)
                            messagebox.showinfo("Ejected", f"Successfully ejected: {sel_display}")
                            dialog.destroy()
                            return
                    ...
                    # Final fallback: attempt sudo umount (may require password)
                    try:
                        subprocess.run(['sudo', 'umount', target], capture_output=True, text=True, check=True)
                        messagebox.showinfo("Ejected", f"Successfully ejected: {sel_display}")
                        dialog.destroy()
                        return
                    except subprocess.CalledProcessError as e:
                        err = e.stderr or e.stdout or str(e)
                        messagebox.showerror("Eject Failed", f"Failed to eject:\n{err}")
                except Exception as e:
                    messagebox.showerror("Error", f"Error ejecting:\n{e}")

            btn_frame = tk.Frame(dialog)
            btn_frame.pack(fill=tk.X, padx=8, pady=8)
            tk.Button(btn_frame, text="Eject", command=do_unmount, bg="#f44336", fg="white", font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=6)
            tk.Button(btn_frame, text="Cancel", command=dialog.destroy, bg="#cccccc", font=("Arial", 11)).pack(side=tk.RIGHT, padx=6)

            dialog.wait_window()

        safe_action(open_eject_dialog)
    def do_reboot():
