...
Ver 1.7.5     2025-10-24
- Fixed 2D/3D full-data accumulation like v0.7
- Fixed colorbar update logic
- Stable dynamic range zmin/zmax handling
"""
...
def read_loop():
    ...
    while True:
        ...
        # Optional: keep buffer bounded for long runs
        if len(x) > 5000:
...
# ------------------ Update Function ------------------
def update(frame):
    global zmin, zmax, colorbar_ref

    if len(x) < 30:  # wait for enough data
        return
    ...
    # Update z limits dynamically
    zmax = max(zmax, np.max(z_new))
    zmin = min(zmin, np.min(z_new))

    # Clear and plot 2D
    ax.clear()
    ax.imshow(im_Migne, extent=[16, 84, 40, 60], alpha=0.1)
    ...
    # --- Colorbar: force manual clim update ---
    from mpl_toolkits.axes_grid1 import make_axes_locatable
    if colorbar_ref is None:
        ...
        colorbar_ref.mappable.set_clim(vmin=zmin, vmax=zmax)
        colorbar_ref.update_normal(contour_ref)

    # --- 3D Plot ---
    axh.clear()
    surf = axh.plot_surface(x_new, y_new, z_new, cmap='jet',
    ...
    # Optional: show z range values on 3D plot
    axh.text2D(0.70, 0.95, f"Z Max: {np.max(z_new):.4f}", transform=axh.transAxes, color="white")
    axh.text2D(0.70, 0.90, f"Z Min: {np.min(z_new):.4f}", transform=axh.transAxes, color="white")




# ------------------ Figure ------------------
fig = plt.figure('Scan System v1.7.5', figsize=[8, 3.8])
...
th_ser.start()
ani = animation.FuncAnimation(fig, update, interval=400, cache_frame_data=False)
canvas.draw_idle()
root.mainloop()
