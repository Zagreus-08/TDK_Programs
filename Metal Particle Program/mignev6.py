...
class GUIClass(PortDefineClass):
    ...
    def scan_start(self):
        ...
        # Disable most controls during scanning (Stop remains enabled)
        try:
            self.disable_controls_during_scanning()
        except Exception:
            pass
    ...
    def started_homing(self):
        ...
        if self.label11.cget("text") in ["Scanning","Going to ScanPos","Unloading"]:
            return
        # Disable interactive controls and start homing
        self.disable_controls_during_homing()
        self.HomeButton.config(state='disabled')
    ...
    def disable_controls_during_homing(self):
        """Disable interactive controls while homing runs."""
        try:
            self.scan.config(state='disabled')
            self.MoveScanButton.config(state='disabled')
            self.MoveCalibButton.config(state='disabled')
            self.UnloadButton.config(state='disabled')
            self.CalibrateButton.config(state='disabled')
            self.XPlusButton.config(state='disabled')
            self.XMinusButton.config(state='disabled')
            self.YPlusButton.config(state='disabled')
            self.YMinusButton.config(state='disabled')
            self.inc_button.config(state='disabled')
            self.dec_button.config(state='disabled')
            self.ClearOffsetButton.config(state='disabled')
            self.scan_area_minus.config(state='disabled')
            self.scan_area_plus.config(state='disabled')
            self.res_100.config(state='disabled')
            self.res_150.config(state='disabled')
    ...
    def disable_controls_during_scanning(self):
        """Disable interactive controls while scanning runs (keep STOP enabled)."""
        try:
            # Disable controls that should not be used during scan
            self.scan.config(state='disabled')
            self.MoveScanButton.config(state='disabled')
            self.MoveCalibButton.config(state='disabled')
            self.UnloadButton.config(state='disabled')
            ...
            self.up_button.config(state='disabled')
            self.down_button.config(state='disabled')
            # Ensure Stop button is available
            try:
    ...
    def update_button_states(self):
        """Enable/disable buttons based on current axis status"""
        axis_status = self.label11.cget("text")
        
        if axis_status == "Home":
            # At HOME: disable XY arrows, disable Z-height offset arrows, disable adjust speed
            self.XPlusButton.config(state='disabled')
            self.XMinusButton.config(state='disabled')
            self.YPlusButton.config(state='disabled')
            self.YMinusButton.config(state='disabled')
            self.inc_button.config(state='disabled')
            self.dec_button.config(state='disabled')
            self.low.config(state='disabled')
            self.med.config(state='disabled')
            self.high.config(state='disabled')
            self.xy_low.config(state='disabled')
            self.xy_med.config(state='disabled')
            # Enable scan area and scanning distance buttons
            self.scan_area_minus.config(state='normal')
            self.scan_area_plus.config(state='normal')
            self.up_button.config(state='normal')
            self.down_button.config(state='normal')
            ...
            # At SCAN POS: enable XY arrows, enable Z-height offset arrows, enable adjust speed
            # Disable area mm and scanning distance buttons
            self.XPlusButton.config(state='normal')
            self.XMinusButton.config(state='normal')
            self.YPlusButton.config(state='normal')
            self.YMinusButton.config(state='normal')
            self.inc_button.config(state='normal')
            self.dec_button.config(state='normal')
            self.low.config(state='normal')
            self.med.config(state='normal')
            self.high.config(state='normal')
            self.xy_low.config(state='normal')
            self.xy_med.config(state='normal')
            # Disable scan area and scanning distance buttons
            self.scan_area_minus.config(state='disabled')
            self.scan_area_plus.config(state='disabled')
            self.up_button.config(state='disabled')
            self.down_button.config(state='disabled')
            ...
            # At CALIBRATION POS: enable only XY arrows, disable everything else
            self.XPlusButton.config(state='normal')
            self.XMinusButton.config(state='normal')
            self.YPlusButton.config(state='normal')
            self.YMinusButton.config(state='normal')
            self.xy_low.config(state='normal')
            self.xy_med.config(state='normal')
            # Disable Z-height offset arrows and adjust speed
            self.inc_button.config(state='disabled')
            self.dec_button.config(state='disabled')
            self.low.config(state='disabled')
            self.med.config(state='disabled')
            ...
            # For all other states: disable all adjustment buttons
            self.XPlusButton.config(state='disabled')
            self.XMinusButton.config(state='disabled')
            self.YPlusButton.config(state='disabled')
            self.YMinusButton.config(state='disabled')
