...
def resume_live():
    ...
    y.clear()
    z.clear()
    # Reset z-axis limits to default (unless locked)
    if not z_range_locked:
        zmin, zmax = -0.1, 0.1
...
if __name__ == '__main__':
    ...
    def open_adjust_dialog():
        ...
        # Ensure entry gets focus when clicked (triggers onboard keyboard)
        def focus_zmin(event):
            zmin_entry.focus_set()
            zmin_entry.select_range(0, tk.END)  # Select all text for easy editing
            # Explicitly trigger onboard keyboard
            try:
                os.system("onboard &")
            except:
                pass
        ...
        def update_zmin(delta):
            try:
                ...
                pass  # Ignore if entry is not a valid number
        ...
        def focus_zmax(event):
            ...
            zmax_entry.select_range(0, tk.END)  # Select all text for easy editing
            # Explicitly trigger onboard keyboard
            try:
                os.system("onboard &")
            except:
