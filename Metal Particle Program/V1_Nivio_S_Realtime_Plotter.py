...
# ---------------- MCC 128 DAQ HAT Local Sensor Reader ----------------
def read_local_sensors():
    """
    Read Z and Z2 sensor values from LOCAL MCC 128 DAQ HAT.
    This is called independently when X,Y coordinates are received via serial.
    
    Returns:
        tuple: (z_voltage, z2_voltage) from channels 0 and 1
    """
    ...
    if mcc128_hat is None:
        print("[WARNING] No MCC 128 DAQ HAT available - returning zero values")
        return (0.0, 0.0)
    
    try:
        # Read from MCC 128 DAQ HAT channels 0 and 1
        ...
        if read_result.data and len(read_result.data) > 0:
            ...
            z_voltage = sum(ch0_data) / len(ch0_data) if ch0_data else 0.0
            z2_voltage = sum(ch1_data) / len(ch1_data) if ch1_data else 0.0
            
            return (z_voltage, z2_voltage)
        else:
            return (0.0, 0.0)
                
    except Exception as e:
        print(f"[ERROR] Local sensor read error: {e}")
        return (0.0, 0.0)
...
def read_loop():
    ...
    while True:
        ...
        try:
            ...
            # Read Z and Z2 from LOCAL MCC 128 DAQ HAT
            z0, z2_value = read_local_sensors()
