...
Ver 1.1-HV-Averaging   2025-01-XX  (Horizontal + Vertical scan with averaging)

FEATURES:
- Auto-detects scan dimensions from hardware (X: 50-300, Y: auto-detected)
- **NEW: Two-phase scanning with averaging**
  * Phase 1: Horizontal scan - collects first pass data
  * Phase 2: Vertical scan - collects second pass data (horizontal remains visible)
  * Automatic averaging: Calculates average Z values for matching (X,Y) coordinates
- Saves raw CSV data for horizontal, vertical, and averaged scans
- Auto-saves PNG for horizontal and averaged results
- Loads and displays any size raw data (50x50 to 300x300)
- Maintains square 2D display with proper axis scaling
- X/Y auto-sync: whichever axis expands, the other follows
...
# Horizontal and Vertical scan data storage
horizontal_data = {'x': [], 'y': [], 'z': []}  # Phase 1: Horizontal scan
vertical_data = {'x': [], 'y': [], 'z': []}    # Phase 2: Vertical scan
averaged_data = {'x': [], 'y': [], 'z': []}    # Combined average data
scan_phase = 'horizontal'  # Current phase: 'horizontal', 'vertical', or 'averaged'
horizontal_complete = False  # Flag to track if horizontal scan is done
...
# ---------------- Raw Data Handling ----------------
def calculate_average_data(h_data, v_data):
    """
    Calculate average Z values for matching (X, Y) coordinates from horizontal and vertical scans.
    Returns a dictionary with averaged x, y, z lists.
    """
    # Create a dictionary to store Z values by (x, y) coordinate
    coord_z_values = {}
    
    # Add horizontal data
    for i in range(len(h_data['x'])):
        coord = (h_data['x'][i], h_data['y'][i])
        if coord not in coord_z_values:
            coord_z_values[coord] = []
        coord_z_values[coord].append(h_data['z'][i])
    
    # Add vertical data
    for i in range(len(v_data['x'])):
        coord = (v_data['x'][i], v_data['y'][i])
        if coord not in coord_z_values:
            coord_z_values[coord] = []
        coord_z_values[coord].append(v_data['z'][i])
    
    # Calculate averages
    avg_data = {'x': [], 'y': [], 'z': []}
    for coord, z_values in coord_z_values.items():
        avg_data['x'].append(coord[0])
        avg_data['y'].append(coord[1])
        avg_data['z'].append(np.mean(z_values))  # Average of all Z values at this coordinate
    
    print(f"[INFO] Averaging complete: {len(coord_z_values)} unique coordinates")
    print(f"[INFO] Horizontal points: {len(h_data['x'])}, Vertical points: {len(v_data['x'])}")
...
def read_loop():
    ...
    global horizontal_data, vertical_data, averaged_data, scan_phase, horizontal_complete
    data_cnt = 0
    ...
    while True:
        ...
        if x0 == 0 and y0 == 0:
            # Check if this is the start of vertical phase (horizontal already complete)
            if horizontal_complete and scan_phase == 'horizontal':
                print("[INFO] Starting VERTICAL scan phase (0,0 detected after horizontal)")
                scan_phase = 'vertical'
                vertical_data = {'x': [], 'y': [], 'z': []}
                data_cnt = 0
                scan_active = True
                
                # Keep horizontal data visible, start collecting vertical
                # Don't clear x, y, z - keep horizontal data displayed
                
                # Start new CSV file for vertical scan
                if filename_from_serial:
                    vertical_filename = filename_from_serial + "_vertical"
                    start_new_raw_file(vertical_filename)
                
                continue
            
            print("[INFO] New HORIZONTAL scan detected (0,0). Resetting everything...")
            scan_phase = 'horizontal'
            ...
            if len(parts) >= 4:
                ...
                start_new_raw_file(filename_from_serial + "_horizontal")
                print(f"[INFO] Started new HORIZONTAL scan with filename: {filename_from_serial}")
        ...
        # Store data based on current phase
        if scan_phase == 'horizontal':
            horizontal_data['x'].append(x0)
            horizontal_data['y'].append(y0)
            horizontal_data['z'].append(z0)
            # Also append to main display buffers for live view
            x.append(x0)
            y.append(y0)
            z.append(z0)
        elif scan_phase == 'vertical':
            vertical_data['x'].append(x0)
            vertical_data['y'].append(y0)
            vertical_data['z'].append(z0)
            # Don't append to main buffers yet - will average later
        ...
        if scan_active and data_cnt > 50 and ((abs(x0 - x_range) <= 1 and abs(y0 - y_max) <= 1) or (x0 == y0 and x0 >= 100 and x0 == y_max)):
            if scan_phase == 'horizontal':
                print(f"[INFO] HORIZONTAL scan complete at ({x0},{y0})")
                print(f"[INFO] Horizontal data points collected: {len(horizontal_data['x'])}")
                horizontal_complete = True
                
                # Save horizontal scan image
                try:
                    if filename_from_serial:
                        possible_paths = ['/home/pi/Shared']
                        save_dir = os.getcwd()
                        for path in possible_paths:
                            if os.path.exists(path):
                                save_dir = path
                                break
                        save_path = os.path.join(save_dir, filename_from_serial + "_horizontal.png")
                        print(f"[INFO] Saving horizontal scan to: {save_path}")
                        root.after(0, lambda: save_figure_direct(save_path))
                except Exception as e:
                    print("[ERROR] Could not save horizontal image:", e)

                if raw_file:
                    try:
                        raw_file.flush()
                        raw_file.close()
                        print(f"[INFO] Horizontal raw data saved: {current_filename}")
                    except Exception as e:
                        print(f"[ERROR] Failed to close horizontal raw file: {e}")
                    raw_file = None
                    csv_writer = None

                # Don't mark scan as inactive - wait for vertical scan
                print("[INFO] Waiting for VERTICAL scan to begin...")
                print("[INFO] Please start vertical scan on the hardware")
                
            elif scan_phase == 'vertical':
                print(f"[INFO] VERTICAL scan complete at ({x0},{y0})")
                print(f"[INFO] Vertical data points collected: {len(vertical_data['x'])}")
                
                # Calculate averaged data
                print("[INFO] Calculating average of horizontal and vertical scans...")
                averaged_data = calculate_average_data(horizontal_data, vertical_data)
                print(f"[INFO] Averaged data points: {len(averaged_data['x'])}")
                
                # Update main display buffers with averaged data
                x.clear()
                y.clear()
                z.clear()
                x.extend(averaged_data['x'])
                y.extend(averaged_data['y'])
                z.extend(averaged_data['z'])
                
                scan_phase = 'averaged'
                
                # Save averaged scan image
                try:
                    if filename_from_serial:
                        possible_paths = ['/home/pi/Shared']
                        save_dir = os.getcwd()
                        for path in possible_paths:
                            if os.path.exists(path):
                                save_dir = path
                                break
                        save_path = os.path.join(save_dir, filename_from_serial + "_averaged.png")
                        print(f"[INFO] Saving averaged scan to: {save_path}")
                        # Small delay to allow plot to update with averaged data
                        root.after(500, lambda: save_figure_direct(save_path))
                except Exception as e:
                    print("[ERROR] Could not save averaged image:", e)

                if raw_file:
                    try:
                        raw_file.flush()
                        raw_file.close()
                        print(f"[INFO] Vertical raw data saved: {current_filename}")
                    except Exception as e:
                        print(f"[ERROR] Failed to close vertical raw file: {e}")
                    raw_file = None
                    csv_writer = None
                ...
                try:
                    if filename_from_serial:
                        avg_csv_path = os.path.join(save_dir, filename_from_serial + "_averaged.csv")
                        with open(avg_csv_path, 'w', newline='') as avg_file:
                            avg_writer = csv.writer(avg_file)
                            avg_writer.writerow(['x', 'y', 'z'])
                            for i in range(len(averaged_data['x'])):
                                avg_writer.writerow([averaged_data['x'][i], averaged_data['y'][i], averaged_data['z'][i]])
                        print(f"[INFO] Averaged data saved to: {avg_csv_path}")
                except Exception as e:
                    print(f"[ERROR] Failed to save averaged CSV: {e}")
            ...
            continue  # Skip the old end-of-scan code below
            
        # OLD END-OF-SCAN CODE (kept as fallback but should not execute)
        if False:  # Disabled - replaced by phase-aware code above
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    if current_filename:
        ...
        # Add phase indicator
        if scan_phase == 'horizontal':
            display_name = f"Live Scan (HORIZONTAL): {base_name}"
        elif scan_phase == 'vertical':
            display_name = f"Live Scan (VERTICAL): {base_name}"
        elif scan_phase == 'averaged':
            display_name = f"Live Scan (AVERAGED): {base_name}"
        else:
