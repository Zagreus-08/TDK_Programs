...
def read_loop():
    ...
    global scan_area_x, scan_area_y, scan_count_x, scan_count_y
    data_cnt = 0
    ...
    while True:
            ...
            if line.startswith("META,"):
                ...
                if len(parts) >= 5:
                    try:
                        ...
                        # Update display ranges based on received area (CRITICAL FIX)
                        x_range = int(scan_area_x)
                        y_max = int(scan_area_y)
                        
                        # Keep them synced for square display
                        max_range = max(x_range, y_max)
                        x_range = y_max = max_range
                        
                        print(f"[INFO] ========== METADATA RECEIVED ==========")
                        print(f"[INFO] Received scan metadata:")
                        print(f"  Area: {scan_area_x}mm x {scan_area_y}mm")
                        print(f"  Scan counts: X={scan_count_x} (max 100), Y={scan_count_y} (max 21)")
                        print(f"  Display range UPDATED to: X=0-{x_range}mm, Y=0-{y_max}mm")
                        print(f"[INFO] =========================================")
                        
                        # If scan counts are specified, calculate actual display coverage
                        if scan_count_x > 0:
                            x_percentage = (scan_count_x / 100.0) * 100
                            actual_x_coverage = (scan_area_x - 10) * (scan_count_x / 100.0)
                            print(f"  X coverage: {actual_x_coverage:.1f}mm ({x_percentage:.0f}% of usable area)")
                        if scan_count_y > 0:
                            y_percentage = (scan_count_y / 21.0) * 100
                            actual_y_coverage = (scan_area_y - 10) * (scan_count_y / 21.0)
                            print(f"  Y coverage: {actual_y_coverage:.1f}mm ({y_percentage:.0f}% of usable area)")
        ...
        # Update x_range and y_max based on ACTUAL data received (not metadata)
        # This ensures the display matches what was actually scanned
        if x0 > 0 and x0 <= 300:  # Reasonable range check for X
            detected_x_max = max(x0, x_range if x_range > 50 else 50)
            # Update x_range if we see larger values
            if detected_x_max > x_range:
                x_range = detected_x_max
                # sync with y_max to keep square scaling
                max_range = max(x_range, y_max)
                x_range = y_max = max_range
                print(f"[INFO] Updated x_range to: {x_range} (synced y_max)")

        if y0 > 0 and y0 <= 300:  # Reasonable range check for Y
            detected_y_max = max(y0, y_max if y_max > 50 else 50)
            # Update y_max if we see larger values
            if detected_y_max > y_max:
                y_max = detected_y_max
                # sync with x_range to keep square scaling
                max_range = max(x_range, y_max)
                x_range = y_max = max_range
                print(f"[INFO] Updated y_max to: {y_max} (synced x_range)")
...
def update(i, xt, yt, zt, zmin_arg, zmax_arg):
    ...
    # Keep x_range and y_max synced before scaling to maintain SQUARE display
    max_range = max(x_range, y_max)
    x_range = y_max = max_range
    ...
    xs = [100 - (xi * 100 / x_range) for xi in xs]  # Invert X: right becomes left
    ys = [100 - (yi * 100 / y_max) for yi in ys]  # Invert Y: bottom becomes top
    ...
    # Adjust ticks to show x_range and y_max in mm (BOTH AXES INVERTED)
    # Display is always 0-100, but labels show actual mm based on area
    # FIX: Use minimum dimension minus 2 for axis labels to fix 291x295 mismatch
    display_x_range = x_range
    display_y_max = y_max
    if x_range != y_max:
        min_dim = min(x_range, y_max) - 2
        display_x_range = display_y_max = max(50, min_dim)
