...
def generate_sensor_id():
    ...
    batch = str(random.randint(10000, 99999))
    serial = str(random.randint(100000, 999999))
    return f"{year}-{month}-{batch}-{serial}"
...
def generate_new_label():
    new_id = generate_sensor_id()
    entry.delete(0, tk.END)
    entry.insert(0, new_id)
    update_preview()

btn_generate_new = tk.Button(btn_frame_top, text="Generate New Label", command=generate_new_label)
btn_generate_new.pack(side=tk.LEFT, padx=5)

btn_generate = tk.Button(btn_frame_top, text="Update Preview")
