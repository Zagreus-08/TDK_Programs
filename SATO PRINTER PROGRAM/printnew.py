...
# -------------------------
root = tk.Tk()
root.title("TDK Sensor Label Generator (draggable)")
...
canvas_items["tdk_logo"] = canvas.create_image(positions_preview["tdk_logo"][0] + 5, # changed from create_text to create_image
...
def update_preview():
    ...
    _global_images["preview_base"] = ph    # ✅ keep alive
    canvas.itemconfig(canvas_bg, image=ph)

    ps = ImageTk.PhotoImage(preview_sensor, master=root)
    _global_images["preview_sensor"] = ps  # ✅ keep alive
    ...
    # Update TDK logo preview
    if preview_tdk_logo:
        pl = ImageTk.PhotoImage(preview_tdk_logo, master=root)
        _global_images["preview_tdk_logo"] = pl # ✅ keep alive
        ...
        # Hide the logo item if it can't be loaded
...
def print_label():
    ...
    if final:
        try:
            ...
            final.save(temp_path, dpi=(DPI, DPI))
            
            # Use os.startfile on Windows to print
            if os.name == 'nt':  # Check if the OS is Windows
                os.startfile(temp_path, "print")
                #messagebox.showinfo("Print", "Label sent to default printer.")
            else:
                # Add cross-platform printing logic here if needed
