...
PREVIEW_W, PREVIEW_H = WIDTH_PX * 1, HEIGHT_PX * 1
...
def load_pillow_fonts(height_px):
    try:
        ...
        font_confidential = ImageFont.truetype("arial.ttf", 35)
...
def make_label1_image():
    ...
    bms_x = (WIDTH_PX - bms_width) // 2 
    bms_y = HEIGHT_PX - 50 - y_offset
...
def make_combined_preview(label1, label2):
    ...
    gap = 10
    combined_height = PREVIEW_H * 2 + gap
    combined = Image.new("RGB", (PREVIEW_W, combined_height), "lightgray")
    combined.paste(preview_label1, (0, 0))
    combined.paste(preview_label2, (0, PREVIEW_H + gap))
...
def generate_ezpl_label(label_image, label_num):
    ...
    commands.append("^W80")    # Set label width (80mm for 20mm at 203dpi)
...
canvas_height = PREVIEW_H * 2 + 20
canvas = tk.Canvas(root, width=PREVIEW_W+10, height=canvas_height, bg="grey")
canvas.pack(pady=10)

# Create blank preview
blank_preview = Image.new("RGB", (PREVIEW_W, canvas_height), "white")
...
def save_label():
    ...
    if fp:
        ...
        canvas.label1_full.save(label1_path, dpi=(DPI, DPI))
        canvas.label2_full.save(label2_path, dpi=(DPI, DPI))
